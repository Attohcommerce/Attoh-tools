...
export async function POST(req) {
  ...
  for (const item of slice) {
    try {
      results.push(await buildForProduct({ domain: store.domain, item, market: market || "USA", aliInput, useSearch, force, fallbackStandard }));
    } catch (e) {
      results.push({ id: item.id, title: item.title, ok: false, verdict: "red", reason: `fout: ${String(e.message || e).slice(0, 160)}` });
    }
  }
  const nextCursor = cursor + slice.length;
  return NextResponse.json({ ok: true, results, nextCursor, done: nextCursor >= items.length });
}
