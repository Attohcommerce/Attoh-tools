...
export async function generateListing({ product, settings }) {
  ...
  const titleKeyword = (isOccasionKw && titleForm) || requiredKeyword;
...
1. TARGET-MARKET stores first, ranked by monthly visits — their best-sellers are already validated on the exact audience.
2. Add OTHER-MARKET stores with high visits: they carry winning products the target market's competitors do NOT sell yet — less competition on the same search demand. English markets (USA/UK/AUS/CANADA) need no translation; NL/BE, FR, PL, DE stores will be searched with translated keywords.
