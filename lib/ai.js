...
CRITICAL: your ENTIRE response must be that single JSON object — no explanations, no image descriptions, no markdown fences, no text before or after the JSON.`
  ...
  const callModel = async (content, maxTokens = 3500) => {
    ...
    // "Geen JSON in AI-antwoord" kwam voor het overgrote deel niet door de
    // foto's, maar door afkapping: bij max_tokens stopt het antwoord midden
    // in description_html, dus zonder sluitende "}" — de regex in
    // extractJson() vindt dan letterlijk niets. Dat is aan stop_reason te
    // herkennen en verdient een groter budget / kortere tekst, geen
    // foto's-loze herkansing (die lost een lengteprobleem niet op).
    if (res.stop_reason === "max_tokens") {
      const err = new Error(`AI-antwoord afgekapt op max_tokens (${raw.length} tekens ontvangen, geen sluitende JSON)`);
      err.truncated = true;
      throw err;
    }
  ...
  // Poging 1 — met foto's. Faalt dat, dan twee mogelijke oorzaken met elk
  // hun eigen herkansing:
  //  - afgekapt op max_tokens → groter budget + expliciete "kort houden"-
  //    instructie (dezelfde foto's, want die waren niet het probleem);
  //  - andere reden (dode afbeeldings-URL, model schiet door foto's uit
  //    JSON-vorm) → exact dezelfde prompt zonder foto's, de maandenlang
  //    betrouwbare basis.
  let listing;
  try {
    ...
    if (e.truncated) {
      listing = await callModel(
        buildContent(imageUrls.length > 0, "Your previous attempt was cut off before finishing the JSON. Keep description_html concise (max ~110 words) and make sure the ENTIRE JSON object closes properly this time."),
        4500
      );
...
- an ASSORTMENT MISFIT: sport-function or niche gear that doesn't belong in a fashion boutique ("volleyball shoes", "cleats", "running spikes", "soccer jersey", "hiking crampons", "swim goggles", scrubs, workwear/PPE). Everyday fashion sneakers, hoodies and joggers are FINE — only flag items bought for the sport/function, not the look.
