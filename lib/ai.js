...
export async function reviewUnderdogPicks(items, opts = {}) {
  ...
  /* Kleine stukken + ruim token-plafond. Bij 150 keywords per call werd het
     antwoord (uitleg-zin per keeper!) langer dan max_tokens, halverwege
     afgekapt → "Geen JSON in AI-antwoord" en de hele run stuk. Nu: 60 per
     call, plafond 8000, en bij een kapot antwoord één herkansing in twee
     helften — een chunk die dan nóg faalt, valt af als "niet beoordeeld"
     in plaats van dat hij de run laat klappen. */
  const CHUNK = 60;
  const picks = [];
  const drop = [];
...
For every keyword you KEEP, write "uitleg": ONE compact sentence in Dutch (max ~20 words) that says exactly WHAT the item is plus 1–2 visual cues to recognise it on a product photo (e.g. "Wijdvallende gehaakte strandjurk — herkenbaar aan het grove open haakwerk."). This sentence is used later by an AI scraper to verify product photos, because competitors rarely use these exact words in their titles. Keep it short: long answers get cut off.
