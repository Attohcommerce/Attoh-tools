...
export async function verifyUnderdogCandidates(keyword, profile, items) {
  ...
  content.push({
    type: "text",
    text: `Return ONLY JSON: {"keep":[{"index":0,"why":"short reason in Dutch"}]} — ONLY the products that genuinely are the item described. Leave out everything else; no "reject" list needed.`,
  });

  const res = await getClient().messages.create({
    model: MODEL,
    max_tokens: 2000,
    system: `You verify whether scraped products really are a specific niche fashion item, judging the TITLE, the DESCRIPTION and the PHOTOS together.

The item we want: "${keyword}"
What it is: ${p.uitleg || p.photoCheck || keyword}
Must have: ${(p.mustHave || []).join(" · ") || "(matches the description above)"}
Never accept if: ${(p.disqualifiers || []).join(" · ") || "(it is clearly a different garment)"}
Photo test: ${p.photoCheck || "The photo must show the item described above."}

The shop will NOT have used these words in its title — judge what the product IS, not what it is called. Be strict: it is far better to keep three certain matches than ten maybes. If the photos do not clearly show the required features, do not keep it.`,
    messages: [{ role: "user", content }],
  });
