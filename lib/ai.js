...
${String(product.bodyHtml || "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").slice(0, 2200)}`,
  ...
  // Poging 1 — met foto's; als de afbeeldings-URLs door de API geweigerd
  // worden (dood/geen toegang), zelfde prompt zonder foto's.
  let listing;
  try {
    listing = await callModel(buildContent(true));
  } catch (e) {
    if (imageUrls.length && /image|url|fetch|media/i.test(String(e.message || e))) {
