  ...
  for (let i = 0; i < items.length; i += CHUNK) {
    ...
    try {
      const res = await getClient().messages.create({
        model: MODEL,
        max_tokens: 4000,
        system: `You prepare NICHE fashion keywords for scraping competitor stores${opts.market ? ` in the ${opts.market} market` : ""}. Shops almost never put these exact words in their product titles, so a literal search fails. For each keyword produce a practical search-and-verify profile.

Return ONLY JSON: {"profiles":[{
  "i": 0,
  "broadType": "swimsuit",
  "searchTerms": ["long sleeve swimsuit","high neck swimsuit","modest swimsuit"],
  "mustHave": ["covers torso and shoulders","long sleeves or high neckline"],
  "disqualifiers": ["bikini","string","plunge neckline","cut-out"],
  "titleForm": "Long Sleeve Swimsuit",
  "photoCheck": "One-piece swimsuit that visibly covers arms or neck — no two-piece, no deep cleavage."
}]}

Rules:
- "broadType": the plain garment type a shop WOULD use in a title (1-2 words, singular or plural as natural).
- "searchTerms": 3-6 phrases that shops genuinely use in titles for this item. Real merchandising language, not the niche phrasing. Order: most likely first. Never include brand names.
- "mustHave": 1-3 concrete, VISUALLY CHECKABLE requirements. No vague words like "elegant" or "stylish".
...
- "photoCheck": ONE sentence telling a photo reviewer exactly what to look for and what rules it out.
Write mustHave/disqualifiers/photoCheck in English. Keep everything short.`,
      ...
      for (const p of parsed.profiles || []) {
        ...
        out[item.kw] = {
          ...
          searchTerms: arr(p.searchTerms).slice(0, 6),
