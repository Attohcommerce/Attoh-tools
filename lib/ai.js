...
export async function reviewVerdelingFinal(rows, market) {
  // rows: [{kw, col, n}] → [{kw, reason}]
  const list = rows.map((r) => `- "${r.kw}" (${r.col}, ${r.n} products)`).join("\n");
  const res = await getClient().messages.create({
    model: MODEL,
    max_tokens: 1200,
    system: `You are the FINAL quality gate for a fashion store's keyword/product plan${market ? ` for the ${market} market` : ""}. Below is the complete chosen plan. Products will be sourced and advertised on Google Shopping under these exact keywords, so every bad keyword wastes real money.
...
Return ONLY JSON: {"remove": [{"kw": "...", "reason": "brand|artifact|market|duplicate"}]} — empty array if the plan is clean.`,
    messages: [{ role: "user", content: list }],
  });
  const out = extractJson(res.content.map((c) => c.text || "").join(""));
  return (out.remove || [])
