...
- "must": 1-4 hard requirements a product MUST meet to qualify (e.g. for "cocktail dress": semi-formal/dressy fabric or finish; hemline roughly knee to mid-calf/midi).
- "typical": 2-5 common but not mandatory traits (fabrics, details, colours).
- "not": 2-5 things that DISQUALIFY, especially the near-misses (e.g. for "cocktail dress": floor-length gowns, plain casual jersey day dresses, bridal white bridal gowns, mini club dresses in stretch jersey).
...
Return ONLY JSON: {"briefs":[{"index":0,"definition":"...","must":["..."],"typical":["..."],"not":["..."],"confusions":["..."]}, ...]}`,
    messages: [{ role: "user", content: list }],
  });
  const out = extractJson(res.content.map((c) => c.text || "").join(""));
  const map = {};
...
- REJECT a different garment category (a cardigan is not a coat, a jumpsuit is not a dress), an accessory (bags, covers, hangers, jewelry), or the wrong gender.
