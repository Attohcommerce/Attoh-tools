...
export function validateListing({ title, descriptionHtml }, { listingStyle, requiredKeyword }) {
  ...
  if (listingStyle === "attribute") {
    ...
    if (t.length < 28 || t.length > 75) issues.push(`title is ${t.length} characters — must be 30–70`);
    ...
    if (t.length < 26 || t.length > 55) issues.push(`title is ${t.length} characters — must be 28–50`);
    if (liCount !== 4) issues.push(`description must have exactly 4 bullets (found ${liCount})`);
...
export function scoreListing({ title, descriptionHtml }, { listingStyle, requiredKeyword }) {
  ...
  if (kw) {
    ...
    if (listingStyle === "stacking") {
      if (hits < 2) {
        score -= 1;
        notes.push(`keyword ${hits}× in omschrijving (stacking-doel: 2-3×)`);
      } else if (hits > 4) {
        score -= 0.5;
      ...
      score -= 1;
      notes.push("keyword komt niet terug in de omschrijving-intro");
    }
  }

  // 3. Titel-lengte sweet spot (Google Shopping toont ±70 tekens)
  if (t.length < 35 || t.length > 70) {
    score -= 0.5;
    notes.push(`titellengte ${t.length} — ideaal is 40-65 tekens`);
  }
