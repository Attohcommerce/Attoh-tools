...
/** Waarden schrijven op een exacte plek (values.update, RAW). */
export async function updateValues(sheetId, range, values) {
  const id = parseSheetId(sheetId);
  await sheetsFetch(
    `${id}/values/${encodeURIComponent(range)}?valueInputOption=RAW`,
    "PUT",
    { values }
  );
  return { ok: true };
...
/** Alle waarden in een bereik (of heel tabblad) wissen — opmaak blijft. */
export async function clearValues(sheetId, range) {
  const id = parseSheetId(sheetId);
  await sheetsFetch(`${id}/values/${encodeURIComponent(range)}:clear`, "POST", {});
  return { ok: true };
}

/** Tabblad dupliceren binnen dezelfde spreadsheet (server-side, geen data-verkeer). */
export async function duplicateTab(sheetId, sourceTabId, newName) {
  const data = await spreadsheetBatchUpdate(sheetId, [
    { duplicateSheet: { sourceSheetId: Number(sourceTabId), insertSheetIndex: 0, newSheetName: newName } },
  ]);
  const props = data.replies && data.replies[0] && data.replies[0].duplicateSheet.properties;
  return { ok: true, tabId: props ? props.sheetId : null, title: props ? props.title : newName };
}

/** Raster van een bestaand tabblad op exacte maat zetten (10M-cellenlimiet). */
export async function resizeTabGrid(sheetId, tabId, rows, cols) {
  await spreadsheetBatchUpdate(sheetId, [
    {
      updateSheetProperties: {
        properties: {
          sheetId: Number(tabId),
          gridProperties: { rowCount: Math.max(2, Math.ceil(rows)), columnCount: Math.max(1, Math.ceil(cols)) },
        },
