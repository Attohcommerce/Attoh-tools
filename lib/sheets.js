...
/** Meerdere kolommen van één tabblad in ÉÉN API-verzoek (values:batchGet).
 *  Bij 180k-rijen-tabbladen scheelt dit tot tien losse roundtrips — de
 *  leesfase was de grootste onvoorspelbare tijdvreter en dus de hoofdverdachte
 *  bij 504-timeouts. Geeft { kolomIndex: [waarden vanaf rij 2] } terug. */
export async function readColumnsBatch(sheetId, tab, colIndices) {
  ...
  const ranges = uniq
    .map((i) => {
      ...
      return `ranges=${encodeURIComponent(`'${tab}'!${L}2:${L}`)}`;
    })
    .join("&");
  const data = await sheetsFetch(`${id}/values:batchGet?majorDimension=COLUMNS&${ranges}`);
  const out = {};
  const vrs = data.valueRanges || [];
  uniq.forEach((colIdx, n) => {
    const vr = vrs[n];
    out[colIdx] = vr && vr.values && vr.values[0] ? vr.values[0] : [];
  });
  return out;
}
