...
import { buildForProduct, isFastSteps, normalizeSteps } from "./sizeguide-run.js";
...
const FAST_CHUNK = 150; // alleen standaardtabellen = rekenwerk → grote stappen
...
export async function startJob(opts) {
  ...
  const job = {
    ...
    steps: normalizeSteps(opts.steps, { useSearch: opts.useSearch, useSource: opts.useSource, fallbackStandard: opts.fallbackStandard }),
  ...
  job.fast = isFastSteps(job.steps);
  const on = Object.keys(job.steps).filter((k) => job.steps[k]);
  pushLog(job, `${job.label}: ${items.length} producten · markt ${job.market} · ${job.fast ? "SNEL (alleen standaardtabellen)" : `stappen aan: ${on.join(", ")} · bron-stores ${job.sourceDomains.length}`} · ${job.backup ? `log: ${job.backup.tab}` : "zonder log"}`, "muted");
  await saveJob(job);
...
export async function tickJob(domain) {
  ...
  try {
    ...
    while (job.cursor < job.items.length && !job.stopRequested && job.status === "running") {
      ...
      const slice = job.items.slice(job.cursor, job.cursor + (job.fast ? FAST_CHUNK : CHUNK));
