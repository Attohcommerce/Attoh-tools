...
const BRAND_PHRASES = [
  ...
  "muck boot", "muck boots", "pac sun", "pacsun jeans",
  // Merken die pas in het sep-dec-venster kwamen bovendrijven (Q4-merken)
  "denim tears", "dandy hoodie", "dandy worldwide", "judy blues",
  "chrome hearts", "apple bottom", "apple bottoms", "mother jeans",
  "mothers jeans", "isabel marant", "saint laurent",
  // KEEN Footwear — "keen" is ook een normaal Engels woord, dus alleen
  // als schoenen-frase blokkeren, nooit als los token
  "keen boots", "keen shoes", "keen sandals", "keen footwear", "keen sneakers",
  // Louboutin's rode zool is een geregistreerd merk — "red bottom(s)" is
  // merk-zoekverkeer, geen kleurbeschrijving.
  "red bottom", "red bottoms",
  // Functie-kleding met batterijen (Milwaukee, Ororo) — geen mode-assortiment
  "heated jacket", "heated jackets", "heated vest", "heated hoodie", "heated gloves",
  "golden goose", "common projects", "axel arigato", "new balance",
...
const BRAND_TOKENS_2 = new Set(
  `timbs timb timberland timberlands bape sp5der spyder nikelab ariat ariats skims skim muck jordans yeezy yeezys coogi milwaukee ororo
northface thenorthface chevy chevrolet lulu coach marant bearpaw baerskin docmart docmarts docmartens
...
const NAV_PATTERNS = [
  ...
  "size guide", "size chart", "track order", "delivery", " on sale", "menswear shop",
...
const GENERIC_EXACT = new Set([
  ...
  // Categorie-navigatie en Planner-artefacten (alleen exact): "shirts & tops"
  // is een winkelmenu-item, "down blouse" is een verminkt "button down blouse"
  // met een fout publiek — een donsblouse bestaat niet ("down jacket" wél).
  "shirts & tops", "shirts and tops", "tops & shirts", "tops and shirts",
  "down blouse", "down blouses",
  "sweater dresser", "dresser sweater",
]);

const EXCEPTIONS = [
  "office wear", "office dress", "office outfit", "office look", "office trousers",
  "roman sandals", "coach jacket", "coach jackets",
  // "slip on shoes" is geen On-sneaker: de merkfrase "on shoes" matchte erin
  // en gooide een 55+-kernproduct (111k/seizoen) weg als merk.
  "slip on", "slip ons", "slip on shoes", "slip on sneakers", "slip on loafers",
  "slip on boots", "pull on", "pull on boots", "step in",
];
...
bag bags handbag handbags tote totes crossbody clutch clutches purse purses
satchel satchels backpack backpacks weekender messenger sling pouch wallet
wallets straw raffia woven hat hats sunhat bucket fedora fedoras visor visors
beanie beanies trucker
swim swimming swimmers bathers cossie cossies togs rashie rashies rash
boardshorts boardies trunks singlet singlets sarong sarongs cover coverups
playsuits ord coord coords overalls dungarees hawaiian resort poolside
chino chinos tailored grey
`
    .split(/\s+/)
    .filter(Boolean)
);
