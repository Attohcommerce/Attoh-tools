...
const SOFT_CLUSTERS = [
  ...
  ["casual", "everyday", "relaxed", "laidback", "weekend", "comfy", "comfort", "comfortable", "cosy", "soft"],
  ...
  ["christmas", "xmas", "festive season", "holiday season"],
  ["spring", "transitional", "trans seasonal"],
...
function zonesFor(p) {
  ...
  const z = {
    title: tok(p.title),
    tt: tok(`${p.productType || ""} ${p.tags || ""} ${p.optionValues || ""}`),
    desc: tok(p.bodyHtml || ""),
    img: tok(
      (p.images || [])
...
/* Functionele/corrigerende kleding: shapewear is GEEN fashion-bodysuit.
   "Shapewear For Women | Adjustable Strap Bodysuit" kwam onder het keyword
   "black bodysuit" de store op — een corrigerend ondergoedproduct tussen de
   mode. Alleen toegestaan als de klant er zélf om vraagt. */
const FUNCTIONAL_BLOCK = new Set([
  "shapewear", "shaper", "shapers", "shaping", "slimming", "compression",
  "girdle", "girdles", "postpartum", "maternity", "nursing",
  "diabetic", "medical", "surgical", "posture", "corrective",
  "bunion", "bunions",
  // "orthopedic" staat hier bewust NIET in: orthopedische sneakers en
  // loafers zijn voor de 55+-doelgroep juist bewezen bestsellers.
]);
...
/* ---------------- Hoofdzelfstandig naamwoord ----------------
   In het Engels bepaalt het LAATSTE kledingwoord wat een product is:
   "blazer dress" is een jurk, "hoodie jacket" is een jas, "sweater dress"
   is een jurk, "cardigan and shorts set" is een set. De matcher keek alleen
   of het woord ergens voorkwam, en zette daardoor twee blazerjurken onder
   "blazer", een gequilte jas onder "hoodie" en acht twee-delige sets onder
   losse kledingstukken. Het type van het keyword moet nu het HOOFDWOORD van
   de titel zijn — niet een bijvoeglijk gebruikt woord ervoor.              */
const HEAD_NOUN = (() => {
  const m = new Map();
  for (const g of TYPE_GROUPS) {
    for (const t of g.terms) if (t.length === 1) m.set(t[0], g.id);
  }
  return m;
})();

/* Schoenen overlappen elkaar: een "Kitten Heel Sandal" en een "Kitten Heel
   Mule" zijn allebei gewoon een kitten heel, en een bootie is een boot. Deze
   paren mogen elkaars hoofdwoord zijn; alle andere combinaties niet. */
const HEAD_COMPATIBLE = new Map([
  ["heels", new Set(["heels", "sandals", "mules", "flats"])],
  ["sandals", new Set(["sandals", "heels", "mules"])],
  ["mules", new Set(["mules", "heels", "sandals"])],
  ["flats", new Set(["flats", "loafers", "mules"])],
  ["loafers", new Set(["loafers", "flats"])],
  ["sneakers", new Set(["sneakers"])],
  ["boots", new Set(["boots"])],
  // Een jurk blijft een jurk, een set blijft een set: geen uitzonderingen.
]);

function headCompatible(kwType, head) {
  if (kwType === head) return true;
  const allowed = HEAD_COMPATIBLE.get(kwType);
  return !!(allowed && allowed.has(head));
}

function headTypeOf(titleTokens) {
  for (let i = titleTokens.length - 1; i >= 0; i--) {
    for (const [w, id] of HEAD_NOUN) {
      if (eqW(titleTokens[i], w)) return id;
    }
  }
  return null;
}
...
export function matchProduct(analysis, p) {
  ...
  // Harde blokkade: shapewear/medisch/corrigerend — geen mode, tenzij de
  // klant er zelf naar zoekt ("orthopedic sneakers" mag dus wél gericht).
  const kwFunctional = analysis.kwTokens.some((t) => FUNCTIONAL_BLOCK.has(t));
  if (!kwFunctional && z.title.some((t) => FUNCTIONAL_BLOCK.has(t))) {
    return null;
  }

  /* Harde blokkade: de titel eindigt op een ánder kledingstuk dan het
     keyword. "Camille | Tailored Blazer Dress" is een jurk, geen blazer;
     "NATALIE | QUILTED HOODIE JACKET" is een jas, geen hoodie; "Sophie
     Cardigan and Shorts Set" is een set, geen vest. */
  if (analysis.typeId) {
    const head = headTypeOf(z.title);
    if (head && !headCompatible(analysis.typeId, head)) return null;
  }
