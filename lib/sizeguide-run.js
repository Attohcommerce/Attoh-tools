...
export async function findAliProduct({ domain, item, aliInput, useSearch = true, force = false }) {
  ...
  if (!s.ok) return { pid: null, reason: s.error, ai };
  ...
  if (!pid) return { pid: null, reason: "kandidaat gevonden maar geen product-ID uit de link te halen", candidates: [cand], ai };
