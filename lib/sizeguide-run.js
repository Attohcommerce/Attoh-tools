...
/**
 * Stappen van de pipeline, los aan/uit te zetten (zie SizeGuidePanel → Instellingen):
 *   ownImage         eigen productfoto die een maattabel is → AI leest (optioneel, AI-kosten)
 *   source           bron-store van de concurrent (optioneel, traag: indexeren + 2–10 s/product)
 *   ali              AliExpress-maattabel ophalen (cache/ID) (optioneel, vaak geblokkeerd)
 *   search           AliExpress zoeken op foto — Apify + AI-match (kost geld, 10–20 s/product)
 *   browser          echte browser bij blokkade — Apify Playwright (kost geld, 20–40 s/product)
 *   fallbackStandard standaardtabel per productsoort als er geen (goede) bron is (aanbevolen aan)
 * Alles uit behalve fallbackStandard = "snel": puur rekenwerk, honderden producten per seconde.
 */
export const STEP_KEYS = ["ownImage", "source", "ali", "search", "browser", "fallbackStandard"];
export function normalizeSteps(steps, legacy = {}) {
  const has = (k) => steps && typeof steps === "object" && steps[k] != null;
  const st = {
    ownImage: has("ownImage") ? !!steps.ownImage : legacy.useSource !== false,
    source: has("source") ? !!steps.source : legacy.useSource !== false,
    ali: has("ali") ? !!steps.ali : true,
    search: has("search") ? !!steps.search : legacy.useSearch !== false,
    browser: has("browser") ? !!steps.browser : true,
    fallbackStandard: has("fallbackStandard") ? !!steps.fallbackStandard : legacy.fallbackStandard !== false,
  };
  if (!st.ali) {
    st.search = false;
    st.browser = false;
...
/** Geen enkele netwerk-/AI-stap aan → alleen standaardtabellen (puur rekenwerk). */
export function isFastSteps(steps, legacy = {}) {
  const st = normalizeSteps(steps, legacy);
  return !st.ownImage && !st.source && !st.ali;
}

/**
 * buildForProduct({domain, item, market, aliInput, useSearch, useSource, sourceDomains, force, fallbackStandard, deadline, steps})
...
export async function buildForProduct({
  ...
  // Stappen los aan/uit (UI: instellingen-menu). `steps` gaat vóór de oude
  // losse vlaggen; ontbreekt een sleutel, dan geldt de oude vlag/default.
  const st = normalizeSteps(steps, { useSearch, useSource, fallbackStandard });
  useSearch = st.search;
  useSource = st.source;
  fallbackStandard = st.fallbackStandard;
  const ai = emptyAiUsage();
  ...
  if (st.ownImage && item.chartImages && item.chartImages.length) {
  ...
  /* ---- 3. AliExpress ---- */
  const m = st.ali ? await findAliProduct({ domain, item, useSearch, force }) : { pid: null, reason: "AliExpress uitgeschakeld", ai: emptyAiUsage() };
  mergeAi(ai, m.ai);
  ...
  if (m.pid || m.link) {
    let c;
    if (m.pid) c = await fetchSizeChart(m.pid, { deadline, browser: st.browser });
    else if (st.browser && browserFetchConfigured()) c = await fetchSizeChartFromLink(m.link, { deadline });
  ...
  if (fallbackStandard) {
    ...
    if (std.ok) {
      result = {
        ...
        guide: std.guide,
        source: "standard",
        fallbackReason: st.ali || st.source || st.ownImage ? result.reason || (result.issues || []).join("; ") || "cijfer te laag" : "alleen standaardtabellen (snelle run)",
        issues: result.issues || [],
      };
    } else if (!st.ali && !st.source && !st.ownImage) {
      // Snelle run: de enige bron was de standaardtabel → die reden tonen
      result = { ...result, reason: `standaardtabel niet mogelijk: ${std.reason}` };
    } else {
      result = { ...result, reason: `${result.reason} · standaardtabel niet mogelijk: ${std.reason}` };
    }
  }
  result.tried = tried;
  result.ai = ai;
