...
/** Vandaag als kalenderdag in Londen ("yyyy-mm-dd") — voor de regel dat de
 *  dag van vandaag NOOIT wordt ingevuld: die is pas morgen compleet. */
export function londonToday() {
  return londonDate(new Date().toISOString());
}
...
/* Eén orderregel kan meerdere productregels hebben; de PDF herhaalt de
   storecode dan per productregel, waardoor de kolomtekst er als
   "0d6n10-zj0d6n10-zj0d6n10-zj" uitkomt. Zo'n exacte herhaling terugbrengen
   naar één code — anders leest de tool elke bill als een nieuwe store. */
...
export async function parseKungfubuyPdf(buffer) {
  ...
  /* FAKE WORKER — geleerd op Vercel: zonder deze regel probeert pdf.js het
     workerbestand zélf dynamisch te laden en dat pad zat niet in de
     serverless bundel ("Setting up fake worker failed: Cannot find module
     …/pdf.worker.mjs"). Het workerbestand hier expliciet importeren doet
     twee dingen tegelijk: het registreert globalThis.pdfjsWorker (waardoor
     pdf.js de dynamische load overslaat) én de letterlijke import zorgt dat
     Vercel's file-tracing het bestand meeneemt in de bundel. */
  ...
  for (let p = 1; p <= doc.numPages; p++) {
    ...
    for (let i = 0; i < anchors.length; i++) {
      ...
      rows.push({
        ...
        store: normalizeStoreCode(byCol.store),
      });
    }
  }
