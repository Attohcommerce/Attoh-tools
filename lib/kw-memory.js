...
export async function getTopKeywords({ market, domain }) {
  let m = normMarket(market);
  if (!m && domain) {
    const p = storeProfile(domain);
    if (p) m = normMarket(p.market);
  }
  if (!m) return { market: null, rows: [] };
  const hit = topCache.get(m);
  if (hit && Date.now() - hit.at < 10 * 60 * 1000) return { market: m, rows: hit.rows, window: hit.window };
  const cfg = await getConfig();
  const mem = cfg[m];
  if (!mem || !mem.sheetId) return { market: m, rows: [], notConfigured: true };
  const values = await readRange(mem.sheetId, `${a1Tab(topTabName(m))}!A1:C2501`).catch(() => []);
  if (!values.length) return { market: m, rows: [] };
  ...
  return { market: m, rows, window: windowLabel };
}
