...
/* ---------- Metafields (Size Guide-module, 02-09-2026) ----------
   Product-metafields via GraphQL: metafieldsSet schrijft tot 25 per call,
   metafieldsDelete ruimt op, en de definities zorgen dat de velden in de
   Shopify-admin zichtbaar/filterbaar zijn. Valt allemaal binnen de
   bestaande write_products-scope. */
...
/**
 * setProductMetafields(store, entries) — entries: [{productId, namespace, key, type, value}]
 * → { ok, written, errors[] }  (value = string; JSON zelf stringifyen)
 */
export async function setProductMetafields(store, entries) {
  const mutation = `mutation SetMf($metafields: [MetafieldsSetInput!]!) {
  ...
  for (let i = 0; i < entries.length; i += 25) {
    const chunk = entries.slice(i, i + 25).map((e) => ({
      ownerId: productGid(e.productId),
      namespace: e.namespace,
      key: e.key,
      type: e.type,
      value: String(e.value),
    }));
    const r = await storeGraphql(store, mutation, { metafields: chunk });
    if (!r.ok) {
      errors.push(r.error);
      continue;
    }
    const ue = (r.data && r.data.metafieldsSet && r.data.metafieldsSet.userErrors) || [];
    for (const u of ue) errors.push(`${(u.field || []).join(".")}: ${u.message}`);
    written += ((r.data && r.data.metafieldsSet && r.data.metafieldsSet.metafields) || []).length;
  }
  return { ok: errors.length === 0, written, errors };
}

/** deleteProductMetafields(store, [{productId, namespace, key}]) → { ok, deleted, errors[] } */
export async function deleteProductMetafields(store, list) {
  const mutation = `mutation DelMf($metafields: [MetafieldIdentifierInput!]!) {
  ...
  for (let i = 0; i < list.length; i += 25) {
    const chunk = list.slice(i, i + 25).map((e) => ({ ownerId: productGid(e.productId), namespace: e.namespace, key: e.key }));
    const r = await storeGraphql(store, mutation, { metafields: chunk });
    if (!r.ok) {
      errors.push(r.error);
      continue;
    }
    const d = r.data && r.data.metafieldsDelete;
    for (const u of (d && d.userErrors) || []) errors.push(u.message);
    deleted += ((d && d.deletedMetafields) || []).filter((x) => x && x.key).length;
  }
  return { ok: errors.length === 0, deleted, errors };
}

/**
 * ensureProductMetafieldDefinitions(store, defs) — defs: [{namespace, key, name, type, description}]
 * Bestaat de definitie al ("TAKEN") → oké. → { ok, created, errors[] }
 */
export async function ensureProductMetafieldDefinitions(store, defs) {
  ...
  const errors = [];
  for (const d of defs) {
    const r = await storeGraphql(store, mutation, {
      definition: {
    ...
    const res = r.data && r.data.metafieldDefinitionCreate;
    const ue = (res && res.userErrors) || [];
    if (res && res.createdDefinition) created++;
    for (const u of ue) {
      if (/TAKEN|already|exists|in use/i.test(`${u.code || ""} ${u.message || ""}`)) continue;
      errors.push(`${d.namespace}.${d.key}: ${u.message}`);
    }
  }
...
/**
 * listProductMetafieldValues(store, {namespace, key, max}) → { ok, values: {productId: value} }
 * Alle producten met de waarde van één metafield (250 per pagina).
 */
export async function listProductMetafieldValues(store, { namespace, key, max = 2000 }) {
  const query = `query Mf($after: String, $ns: String!, $key: String!) {
  ...
  for (let page = 0; page < 20; page++) {
    const r = await storeGraphql(store, query, { after, ns: namespace, key });
    if (!r.ok) return { ok: false, error: r.error };
    const p = r.data && r.data.products;
    for (const n of (p && p.nodes) || []) {
      const id = String(n.id).replace(/^gid:\/\/shopify\/Product\//, "");
      if (n.metafield && n.metafield.value != null) values[id] = n.metafield.value;
    }
    if (!p || !p.pageInfo || !p.pageInfo.hasNextPage || Object.keys(values).length >= max) break;
    after = p.pageInfo.endCursor;
  }
  return { ok: true, values };
}

/** getProductMetafieldValues(store, ids, {namespace, key}) → { ok, values: {productId: value} } — één GraphQL-call per 50 id's */
export async function getProductMetafieldValues(store, ids, { namespace, key }) {
  const query = `query MfByIds($ids: [ID!]!, $ns: String!, $key: String!) {
    nodes(ids: $ids) { ... on Product { id metafield(namespace: $ns, key: $key) { value } } }
  }`;
  const values = {};
  const list = (ids || []).map((i) => productGid(i));
  for (let i = 0; i < list.length; i += 50) {
    const r = await storeGraphql(store, query, { ids: list.slice(i, i + 50), ns: namespace, key });
    if (!r.ok) return { ok: false, error: r.error };
    for (const n of (r.data && r.data.nodes) || []) {
      if (!n || !n.id) continue;
      const id = String(n.id).replace(/^gid:\/\/shopify\/Product\//, "");
      if (n.metafield && n.metafield.value != null) values[id] = n.metafield.value;
    }
