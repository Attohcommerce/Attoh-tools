...
export function runDoctor(products, opts = {}) {
  ...
  if (opts.market === "USA" || opts.market === "CAN") {
    ...
    for (const p of products) {
      ...
      if (productSummary(p).gender !== "men") continue;
      ...
      if (small.length && small.length < vs.length) {
        smallHits.push(label(p, `maten ${[...new Set(small.map((v) => optValue(v, idx)))].join("/")} (${small.length} van ${vs.length} varianten)`));
      }
    }
    add(
      "men-shoes-under-6", "error", "Herenschoenen met maten onder US 6",
      "US-herenschoenen beginnen bij 6; kleinere nummers zijn omrekenrestanten en verwarren klanten (en de maattabel). De fix verwijdert precies die varianten (backup vooraf) — daarna de maattabel opnieuw laten bouwen (Size Guide / fix hieronder).",
      smallHits,
      [{ id: "delete-small-men-shoe-sizes", label: "Verwijder maten onder US 6", danger: true }]
    );
  }
...
export async function applyDoctorFix(store, fixId, p, options = {}) {
  ...
  switch (fixId) {
    case "delete-small-men-shoe-sizes": {
      ...
      if (!small.length) return out(false, "geen maten onder US 6");
      if (small.length === vs.length) return out(false, "ALLE maten onder US 6 — overgeslagen (dan klopt er meer niet aan dit product)");
