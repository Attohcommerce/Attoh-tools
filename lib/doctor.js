...
import { familyOf, analyzeSizes, resolveSizeSet } from "@/lib/sizes";
import { productSummary, checkGuide, standardGuide, KIND_LABEL, SG_NS, SG_KEY, SG_STATUS_KEY } from "@/lib/sizeguide";
import { setProductMetafields } from "@/lib/shopify";
...
// Realistisch minimum voor US-schoenmaten: heren 6, dames 4 (daaronder = omrekenrestant)
function shoeMinUs(p) {
  return productSummary(p).gender === "men" ? 6 : 4;
}
/** "S (60-70Kg)" → "S" · "M (almost sold out)" → "M" · "8.5 US" → "8.5" · "XL" → ongewijzigd */
export function cleanSizeLabel(raw) {
  const src = String(raw == null ? "" : raw);
  let v = src.replace(/\s*[\(\[（].*$/, ""); // alles vanaf de eerste haak weg
  v = v.replace(/\b(almost\s+)?sold\s*out\b|\bin\s*stock\b|\bonly\s+\d+\s+left\b/gi, "");
  v = v.replace(/\s*[-–|:]\s*$/, "").replace(/^\s*[-–|:]\s*/, "").replace(/\s{2,}/g, " ").trim();
  if (!v) v = src.trim();
  return { value: v, changed: v !== src.trim() && v !== src };
}
...
function sizeRank(v) {
  ...
  // hasOwnProperty i.p.v. `in`: een waarde "constructor" zit via
  // Object.prototype wél "in" elk gewoon object (zie verdeling.js, canonKey).
  if (Object.prototype.hasOwnProperty.call(SIZE_ORDER, s)) return SIZE_ORDER[s];
...
export function runDoctor(products, opts = {}) {
  ...
  for (const p of products) {
    ...
    if (si >= 0) {
      ...
      if (sizes.length) {
        ...
        const a = analyzeSizes(sizes, { family: fam, gender: gen, market: opts.market || "", bareSystem: opts.bareSystem || "" });
  ...
  add(
    "size-system", "error", "Maten in het verkeerde systeem of met rommel-labels",
    opts.market
      ? `${opts.bareSystem ? `Kale nummers worden als ${String(opts.bareSystem).toUpperCase()}-maten gelezen en naar ${opts.market} omgerekend (dameskleding US 6 → AU 10, "S(4-6)" → "S(8-10)", herenschoen US 10 → AU 9; damesschoenen AU = US, taille-inches en lettermaten blijven). Daarnaast: ` : ""}EU-maten en supplier-mixen als "EU 38/US 7.5" — een ${opts.market}-shopper kent ze niet of raakt in de war: verkeerde maat = retour. De fix denkt als de klant, niet als een rekenmachine: staat het ${opts.market}-nummer er al ergens bij (bv. een US-label op een AU-damesschoen), dan wordt dát nummer overgenomen; anders wordt omgerekend vanaf het juiste gelabelde systeem; het resultaat is altijd één schoon nummer per maat. Elk resultaat moet binnen de logische maat-band vallen — producten met een onduidelijke mix worden NIET aangeraakt en staan hieronder als "handmatig". Schoenentabellen kunnen per merk een halve maat afwijken — doe na de fix een steekproef.`
      : "Deze producten dragen EU-maten of rommel-labels. Kies bovenaan eerst de doelmarkt, dan lost de fix ze veilig op.",
    sizeHits,
    opts.market
      ? [{ id: "convert-sizes", label: `Maak maten klant-logisch (${opts.market})` }]
      : []
  );
  ...
  /* ---------- KLEUR-WAARDEN & SUPPLIER-LEAKS ---------- */

  const COLOR_JUNK_RE = /^(as\s*(picture|photo|shown|pic)|photo\s*colou?r|picture\s*colou?r|same\s*as\s*(photo|picture)|multi|multicolou?r|mixed|random|colou?r\s*\d+|style\s*\d+|[a-h]|\d{1,2})$/i;
  const colorJunkHits = [];
  for (const p of products) {
    const ci = colorOptionIndex(p);
    if (ci < 0) continue;
    const vals = [...new Set((p.variants || []).map((v) => optValue(v, ci)).filter(Boolean))];
    const junk = vals.filter((x) => COLOR_JUNK_RE.test(String(x).trim()));
    if (junk.length) colorJunkHits.push(label(p, junk.slice(0, 4).join(", ")));
  }
  add(
    "color-junk", "warn", "Kleur-waarden zeggen niets",
    '"Photo Color", "As Picture", "Style 2" of "A/B" als kleurnaam — de shopper weet niet wat hij kiest en het color-attribuut in de feed is waardeloos. Geen auto-fix (echte kleurnamen vereisen de foto\'s): hernoem in Shopify, de kleur↔foto-AI-check hieronder helpt kiezen.',
    colorJunkHits
  );

  const LEAK_RE = /\b(aliexpress|ali\s+express|alibaba|dhgate|temu|shein|taobao|1688|banggood|lazada|wish\.com|cj\s?dropshipping)\b/i;
  add(
    "supplier-leak", "error", "Leveranciers-/marketplace-naam zichtbaar",
    "AliExpress, Temu, Shein e.d. in titel, omschrijving, tags of vendor — dodelijk voor vertrouwen én een klassieke misrepresentation-trigger in GMC. Handmatig herschrijven (automatisch wegknippen sloopt zinnen).",
    products
      .filter((p) => LEAK_RE.test([p.title, txt(p.body_html).slice(0, 2000), p.tags, p.vendor].join(" ")))
      .map((p) => {
        const m = [p.title, txt(p.body_html).slice(0, 2000), p.tags, p.vendor].join(" ").match(LEAK_RE);
        return label(p, m ? m[0] : "");
      })
  );
  ...
  for (const p of products) {
    ...
    const prices = new Set(vs.map((v) => String(v.price)));
    const cmps = new Set(vs.map((v) => String(v.compare_at_price || "")));
    if (prices.size > 1) spreadHits.push(label(p, [...prices].slice(0, 4).join(" / ")));
    else if (cmps.size > 1) spreadHits.push(label(p, "doorstreepprijs wisselt per maat"));
  }
  add(
    "price-spread", "warn", "Verschillende prijzen binnen één product",
    "Supplier-prijzen per maat lekken door: de klant ziet per maat een andere prijs (verwarring, minder conversie) en de feed krijgt ruis. De fix zet alle varianten op de meest voorkomende prijs (gelijkspel → laagste) met dezelfde doorstreepprijs.",
    spreadHits,
    [{ id: "unify-variant-prices", label: "Trek variant-prijzen gelijk", bulk: true }]
  );
  ...
  for (const p of products) {
    for (const v of p.variants || []) {
      ...
      bySku.get(sku).add(p.id);
    }
  }
  const dupSkuOf = new Map();
  for (const [sku, idSet] of bySku) {
    if (idSet.size > 1) for (const id of idSet) if (!dupSkuOf.has(id)) dupSkuOf.set(id, sku);
  }
  add(
    "dup-sku", "warn", "Zelfde SKU bij meerdere producten",
    "Dubbele SKU's (meestal mee-gescraped) maken feed-koppeling en rapportages onbetrouwbaar — duidt vaak ook op dubbel geïmporteerde producten.",
    products.filter((p) => dupSkuOf.has(p.id)).map((p) => label(p, `SKU ${dupSkuOf.get(p.id)}`))
  );
  ...
  /* ---------- HERENSCHOENEN ONDER US 6 (Justin, 03-09-2026) ----------
     Op de US/CAN-stores bestaan herenschoenmaten onder US 6 niet — dat zijn
     restanten van EU-omrekeningen of dames-leesten. Weg ermee (variant
     verwijderen, met backup); de maattabel volgt daarna de website. */
  if (opts.market === "USA" || opts.market === "CAN") {
    const smallHits = [];
    for (const p of products) {
      if (familyOf(p.product_type, p.title) !== "shoes") continue;
      const idx = sizeOptionIndex(p);
      if (idx < 0) continue;
      const min = shoeMinUs(p);
      const vs = p.variants || [];
      const small = vs.filter((v) => {
        const n = parseFloat(String(optValue(v, idx) || "").replace(",", "."));
        return !isNaN(n) && n >= 1 && n < min;
      });
      if (small.length && small.length < vs.length) {
        smallHits.push(label(p, `maten ${[...new Set(small.map((v) => optValue(v, idx)))].join("/")} onder US ${min} (${small.length} van ${vs.length} varianten)`));
      }
    }
    add(
      "shoes-under-minimum", "error", "Schoenmaten onder het US-minimum (heren < 6, dames < 4)",
      "US-herenschoenen beginnen bij 6, damesschoenen bij 4; kleinere nummers zijn omrekenrestanten (EU 33–35) en verwarren klanten én de maattabel. De fix verwijdert precies die varianten (backup vooraf) — daarna de maattabel opnieuw laten bouwen (Size Guide / fix hieronder).",
      smallHits,
      [{ id: "delete-small-men-shoe-sizes", label: "Verwijder maten onder het minimum", danger: true }]
    );
  }

  /* ---------- ROMMEL IN MAATLABELS ----------
     "S (60-70Kg)", "M (almost sold out)" — de maat staat vol met leverancierstekst.
     Dat staat in de kiezer én in de maattabel. Fix: alles vanaf de eerste
     haak/rommel weg, met botsings-guard. */
  {
    const junkHits = [];
    for (const p of products) {
      const idx = sizeOptionIndex(p);
      if (idx < 0) continue;
      const bad = (p.variants || []).map((v) => optValue(v, idx)).filter((val) => val != null && cleanSizeLabel(val).changed);
      if (bad.length) junkHits.push(label(p, [...new Set(bad)].slice(0, 4).join(" · ")));
    }
    add(
      "size-label-junk", "error", "Rommel in maatlabels",
      "Maten als \"S (60-70Kg)\" of \"M (almost sold out)\" — leverancierstekst in de variantkiezer. De fix houdt alleen de maat over (S, M, 8.5) en slaat producten over waar twee maten zouden botsen.",
      junkHits,
      [{ id: "clean-size-labels", label: "Schoon maatlabels op", bulk: true }]
    );
  }

  /* ---------- MAATTABELLEN (Size Guide-module, 02-09-2026) ----------
     opts.guides = { productId: guide-JSON } uit metafield custom.size_guide.
     Geen exacte maten-kennis per product (dat vraagt een geheugen voor
     duizenden producten) — wél: hoort de tabel bij de productSOORT
  ...
  if (opts.guides) {
    ...
    const sgStandard = [];
    for (const p of products) {
      const summ = productSummary(p);
      const raw = opts.guides[String(p.id)];
      let guide = null;
      if (raw) {
        try {
          guide = typeof raw === "string" ? JSON.parse(raw) : raw;
        } catch {
          guide = null;
        }
      }
      const c = checkGuide(guide, summ, opts.market || "");
      const kindTxt = KIND_LABEL[c.kind] || c.kind;
      if (c.level === "skip") continue;
      if (c.level === "missing") sgMissing.push(label(p, `${kindTxt} · maten ${summ.sizes.slice(0, 5).join("/")}`));
      else if (c.level === "error") sgWrong.push(label(p, `${kindTxt}: ${c.issues.join("; ")}`));
      else if (c.level === "warn") sgWrong.push(label(p, `${kindTxt}: ${c.issues.join("; ")}`));
      else if (c.standard) sgStandard.push(label(p, kindTxt));
    }
    add(
      "size-guide-missing", "error", "Geen maattabel",
      "Product met maten maar zonder maattabel (metafield custom.size_guide). Klanten kunnen hun maat niet kiezen → conversie. De fix zet een standaardtabel passend bij de productsoort; de Size Guide-pagina haalt daarna de echte leveranciersmaten op.",
      sgMissing,
      [{ id: "fix-size-guides", label: "Zet standaardtabel per productsoort", bulk: true }]
    );
    add(
      "size-guide-wrong", "error", "Maattabel past niet bij het product",
      "Verkeerde maten voor de productsoort (bv. binnenbeenlengte op een blouse, borstomvang op een rok, kledingtabel op een schoen), rijen die niet op de variantmaten passen, of verkeerd geslacht/markt. Dit is precies wat een klant laat afhaken. De fix vervangt 'm door een passende standaardtabel; daarna via Size Guide opnieuw matchen voor de echte maten.",
      sgWrong,
      [{ id: "fix-size-guides", label: "Vervang door passende standaardtabel", bulk: true }]
    );
    findings.push({
      id: "size-guide-standard", level: "warn", soft: true,
      title: "Alleen een standaardtabel (geen leveranciersmaten)",
      why: "Bewust toegestaan als vangnet — telt niet mee in de score. Echte maten ophalen: Size Guide-pagina → Scan store → Bouw maattabellen (eerste toggle aan). Die haalt per product de tabel van de bron-store (de concurrent waar het product vandaan komt) of van AliExpress via een echte browser; wat geen match heeft blijft standaard.",
      count: sgStandard.length, examples: sgStandard.slice(0, 10), ids: [...new Set(sgStandard.map((h) => h.id))], fixes: [],
    });
    if (!sgStandard.length) findings.pop();
  }
  ...
  const errors = findings.filter((f) => f.level === "error" && !f.soft).reduce((s, f) => s + f.count, 0);
  const warns = findings.filter((f) => f.level === "warn" && !f.soft).reduce((s, f) => s + f.count, 0);
...
export async function applyDoctorFix(store, fixId, p, options = {}) {
  ...
  switch (fixId) {
    case "delete-small-men-shoe-sizes": {
      const idx = sizeOptionIndex(p);
      if (idx < 0) return out(false, "geen Size-optie");
      if (familyOf(p.product_type, p.title) !== "shoes") return out(false, "geen schoen");
      const min = shoeMinUs(p);
      const vs = p.variants || [];
      const small = vs.filter((v) => {
        const n = parseFloat(String(optValue(v, idx) || "").replace(",", "."));
      ...
      if (!small.length) return out(false, `geen maten onder US ${min}`);
      if (small.length === vs.length) return out(false, `ALLE maten onder US ${min} — overgeslagen (dan klopt er meer niet aan dit product)`);
      backup.push({ field: "variants (verwijderd)", old: small.map((v) => ({ id: v.id, size: optValue(v, idx), price: v.price, sku: v.sku })) });
      let done = 0;
      for (const v of small) {
        const r = await deleteVariant(store, p.id, v.id);
        if (r.ok) done++;
      }
      return out(done > 0, `${done}/${small.length} maat/maten verwijderd (${small.map((v) => optValue(v, idx)).join("/")}) — maattabel opnieuw bouwen`);
    }
    ...
    case "clean-size-labels": {
      ...
      const plan = (p.variants || []).map((v) => ({ v, c: v[key] != null ? cleanSizeLabel(v[key]) : { value: v[key], changed: false } }));
      if (!plan.some((x) => x.c.changed)) return out(false, "maatlabels waren al schoon");
      // botsings-guard: twee varianten die op hetzelfde label uitkomen → handmatig
      const seen = new Set();
      for (const x of plan) {
        const k = String(x.c.value).toLowerCase();
        if (seen.has(k)) return out(false, `botsing na opschonen (${x.c.value} dubbel) — handmatig`);
        seen.add(k);
      }
      backup.push({ field: "variant-maten", old: (p.variants || []).map((v) => [v.id, v[key]]) });
      // Shopify-valkuilen: hele variant-set meesturen; geraakte varianten met ál hun optie-waarden
      const newVariants = plan.map((x) => {
        if (!x.c.changed) return { id: x.v.id };
        const nv = { id: x.v.id };
        for (const k of ["option1", "option2", "option3"]) if (x.v[k] != null) nv[k] = k === key ? x.c.value : x.v[k];
        return nv;
      });
      const r = await updateProduct(store, p.id, { variants: newVariants });
      if (!r.ok) throw new Error(r.error);
      const n = plan.filter((x) => x.c.changed).length;
      return out(true, `${n} maatlabel(s) opgeschoond (${plan.filter((x) => x.c.changed).slice(0, 3).map((x) => `${x.v[key]} → ${x.c.value}`).join(", ")})`);
    }

    case "fix-size-guides": {
      // Standaardtabel passend bij de productsoort (rok = waist/hips, blouse =
      // bust/waist, schoen = US/EU/UK + voetlengte, …). Handmatig bewerkte
      // tabellen (status manual) blijven staan.
      const summ = productSummary(p);
      const oldRaw = p.__sizeGuide || null;
      if (oldRaw) {
        try {
          const old = JSON.parse(oldRaw);
          if (old && old.status === "manual") return out(false, "handmatige tabel — overgeslagen");
        } catch {}
      }
      const std = standardGuide(summ, options.market || "USA");
      if (!std.ok) return out(false, `geen standaardtabel: ${std.reason}`);
      backup.push({ field: "size_guide", old: oldRaw });
      const w = await setProductMetafields(store, [
        { productId: p.id, namespace: SG_NS, key: SG_KEY, type: "json", value: JSON.stringify(std.guide) },
        { productId: p.id, namespace: SG_NS, key: SG_STATUS_KEY, type: "single_line_text_field", value: "standard" },
      ]);
      if (!w.ok) throw new Error(w.errors.join("; "));
      return out(true, `${KIND_LABEL[std.guide.kind] || std.guide.kind}: ${std.guide.columns.filter((c) => c.kind === "length").map((c) => c.label).join("/")}`);
    }
    ...
    case "translate-options": {
      ...
      let valChanges = 0;
      /* LET OP (tweede Shopify-valkuil, gevonden op de live run): zodra een
         variant-entry ÉÉN optie-waarde meestuurt, eist Shopify ze ALLEMAAL —
         anders "You need to add option values for Color". Dus: raakt een
         variant een optie, dan gaan al z'n optie-waarden mee; onaangepaste
         varianten blijven kaal ({id}). */
      const newVariants = (p.variants || []).map((v) => {
        ...
        for (const k of ["option1", "option2", "option3"]) {
          if (v[k] == null) continue;
          const r = translateValue(v[k]);
          vals[k] = r.changed ? r.value : v[k];
          if (r.changed) {
            touched = true;
            valChanges++;
          }
        }
        if (touched) Object.assign(nv, vals);
        return nv;
      });
    ...
    case "convert-sizes": {
      /* KLANT-LOGICA, GEEN REKENMACHINE (fix van de "maat −26.5"-bug):
         eerst het hele product doorrekenen met resolveSizeValue — die pakt
         het gelabelde doelmarkt-nummer als dat er al staat en rekent alleen
         anders om, met harde sanity-banden. Eén waarde onoplosbaar of een
         botsing? Dan blijft het HELE product ongemoeid ("handmatig") —
         op maten wordt nooit gegokt. */
      ...
      // hele set in één keer oplossen — zo leert de fix ook de product-eigen
      // EU-ladder uit waarden die beide systemen dragen ("EU 35/US 5.5")
      const rawList = (p.variants || []).map((v) => (v[key] == null ? "" : v[key]));
      const setRes = resolveSizeSet(rawList, { family: fam, gender: gen, market, bareSystem: options.bareSystem || "" });
      const plan = (p.variants || []).map((v, i) => ({
        v,
        r: v[key] != null ? setRes[i] : null,
      }));
      if (plan.some((x) => x.r && !x.r.ok)) {
        return out(false, "onduidelijke maten-mix — handmatig aanpassen (op maten gok ik niet)");
      }
      const changes = plan.filter((x) => x.r && x.r.changed).length;
      if (!changes) return out(false, "maten stonden al goed");
      // botsings-guard op de EINDWAARDEN ("38" én "EU 38" → allebei "8")
      const seen = new Set();
      for (const x of plan) {
        const comb = ["option1", "option2", "option3"]
          .map((k) => ((k === key && x.r ? x.r.value : x.v[k]) ?? ""))
          .join("|||");
        if (seen.has(comb)) {
          return out(false, "conversie zou dubbele varianten geven — handmatig (dubbele maat in de bron?)");
        }
        seen.add(comb);
      }
      backup.push({ field: "size-waarden", old: (p.variants || []).map((v) => [v.id, v[key]]) });
      /* Volledige set meesturen (valkuil 1) én per aangeraakte variant ALLE
         optie-waarden meegeven (valkuil 2): stuur je alleen de maat-optie,
         dan weigert Shopify met "You need to add option values for Color". */
      const newVariants = plan.map((x) => {
        const nv = { id: x.v.id };
        if (x.r && x.r.changed) {
          for (const k of ["option1", "option2", "option3"]) {
            if (x.v[k] == null) continue;
            nv[k] = k === key ? x.r.value : x.v[k];
          }
        }
        return nv;
      });
      const r = await updateProduct(store, p.id, { variants: newVariants });
      if (!r.ok) throw new Error(r.error);
      const toTxt = plan.filter((x) => x.r).map((x) => x.r.value).slice(0, 8).join("/");
      return out(true, `${changes} maten → ${toTxt} (${market})`);
    }

    case "unify-variant-prices": {
      const vs = p.variants || [];
      if (vs.length < 2) return out(false, "één variant");
      const priceOf = (v) => String(v.price);
      const count = new Map();
      for (const v of vs) count.set(priceOf(v), (count.get(priceOf(v)) || 0) + 1);
      const cmpSet = new Set(vs.map((v) => String(v.compare_at_price || "")));
      if (count.size < 2 && cmpSet.size < 2) return out(false, "prijzen waren al gelijk");
      // referentie: de meest voorkomende prijs, bij gelijkspel de laagste
      const best = [...count.entries()].sort((a, b) => b[1] - a[1] || Number(a[0]) - Number(b[0]))[0][0];
      const ref = vs.find((v) => priceOf(v) === best);
      const refCmp = ref && ref.compare_at_price ? String(ref.compare_at_price) : null;
      const needs = (v) => priceOf(v) !== best || String(v.compare_at_price || "") !== String(refCmp || "");
      if (!vs.some(needs)) return out(false, "stond al gelijk");
      backup.push({ field: "prijzen", old: vs.map((v) => [v.id, v.price, v.compare_at_price]) });
      // volledige set meesturen — zie de Shopify-valkuil bij translate-options
      const r = await updateProduct(store, p.id, {
        variants: vs.map((v) =>
          needs(v) ? { id: v.id, price: best, compare_at_price: refCmp } : { id: v.id }
        ),
      });
      if (!r.ok) throw new Error(r.error);
      return out(true, `alle ${vs.length} varianten → ${best}${refCmp ? ` (doorstreep ${refCmp})` : ""}`);
    }
