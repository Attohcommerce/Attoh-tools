...
import { canonOptionName, translateValue, foreignTextHits, looksEuSizing } from "@/lib/lang";
...
export function runDoctor(products, opts = {}) {
  ...
  add(
    ...
    orphanHits,
    [
      { id: "relink-photos", label: "Her-koppel foto's (gratis)" },
  ...
  add(
    ...
    [{ id: "fill-alt", label: "Vul alt-teksten (titel + kleur)" }]
  );
  ...
  for (const p of products) {
    ...
    if (si >= 0) {
      ...
      if (looksEuSizing(sizes)) euHits.push(label(p, sizes.slice(0, 6).join("/")));
    }
  }
  add(
  ...
  add(
    "eu-sizing", "warn", "EU-maatvoering (34/36/38…)",
    `Numerieke EU-maten in een ${opts.market || "EN"}-store verwarren shoppers én de feed. Automatisch omrekenen is riskant (denim-tailles!) — handmatig beoordelen.`,
    euHits
  );
  ...
  add(
    ...
    compareIssues,
    [{ id: "fix-compareat", label: "Herstel doorstreepprijzen" }]
  );
  ...
  add(
    ...
    'Titel zegt "Women\'s" maar de tag is Men (of andersom) — draai de AI-geslachtscheck om dit in bulk recht te zetten.',
    products
      .filter((p) => {
  ...
  if (opts.vendorName) {
    add(
      ...
      [{ id: "set-vendor", label: `Zet vendor op "${opts.vendorName}"` }]
    );
  }
  ...
  add(
    ...
    [{ id: "set-product-type", label: "Vul product type uit titel" }]
  );
  ...
  add(
    ...
    [{ id: "clear-barcodes", label: "Maak barcodes leeg" }]
  );
  ...
  add(
    ...
    [{ id: "fix-men-template", label: `Zet template "${menTemplate}"` }]
  );
  add(
    ...
    products
      ...
      .map((p) => label(p)),
    [{ id: "fix-women-template", label: "Haal men-template weg" }]
  );
  ...
  add(
    ...
    orderHits,
    [{ id: "fix-size-order", label: "Sorteer maten" }]
  );
  ...
  add(
    ...
    [{ id: "publish-products", label: "Publiceer op Online Store" }]
  );
...
export async function applyDoctorFix(store, fixId, p, options = {}) {
  ...
  switch (fixId) {
    ...
    case "gender-tags": {
      const labelWanted = options.labels && options.labels[String(p.id)];
      if (!labelWanted) return out(false, "geen label voor dit product");
      const tags = tagsOf(p);
      const cleaned = tags.filter((t) => !/^(men|women)$/i.test(t));
      cleaned.push(labelWanted);
      if (cleaned.join(",") === tags.join(",")) return out(false, "tag stond al goed");
      backup.push({ field: "tags", old: p.tags || "" });
      const r = await updateProduct(store, p.id, { tags: cleaned.join(", ") });
      if (!r.ok) throw new Error(r.error);
      return out(true, `tag → ${labelWanted}`);
    }
