...
/* Bestaande canons één keer voorbereiden. De familie-check draait over
   ~180.000 kandidaten × ~100 bestaande keywords; zonder voorbereiding werd
   elke bestaande canon 180.000 keer opnieuw gesplitst en in een Set gestopt —
   dat vrat in z'n eentje seconden van het tijdsbudget dat de AI-lagen
   nodig hebben. */
export function prepFamilies(existingCanons) {
  const canonSet = existingCanons instanceof Set ? existingCanons : new Set(existingCanons);
  const families = [...canonSet].map((c) => {
    const parts = c.split(" ");
    return { set: new Set(parts), len: parts.length };
  });
  return { canonSet, families };
}

export function isFamilyOfExisting(canon, prepped) {
  if (!canon) return true;
  const p = prepped && prepped.families ? prepped : prepFamilies(prepped || new Set());
  if (p.canonSet.has(canon)) return true;
  const parts = canon.split(" ");
  const n = parts.length;
  let candSet = null;
  for (const ex of p.families) {
    if (Math.abs(ex.len - n) > 1) continue;
    if (n <= ex.len) {
      let sub = true;
      for (const s of parts) if (!ex.set.has(s)) { sub = false; break; }
      if (sub) return true;
    } else {
      if (!candSet) candSet = new Set(parts);
      let sub = true;
      for (const s of ex.set) if (!candSet.has(s)) { sub = false; break; }
      if (sub) return true;
    }
  }
  return false;
