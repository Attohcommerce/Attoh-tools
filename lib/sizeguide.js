...
export function buildGuide({ chart, product, market, source = "aliexpress", confidence = null }) {
  ...
  const guide = {
    ...
    note: "Garment measurements, taken flat. Allow 0.5–1 in (1–2 cm) variance.",
...
export function standardGuide(product, market) {
  ...
  if (family === "shoes") {
    ...
    columns = [
      ...
      { key: "bust", kind: "length", label: gender === "men" ? "Chest" : "Bust" },
      { key: "waist", kind: "length", label: "Waist" },
    ];
    if (gender === "women") columns.push({ key: "hip", kind: "length", label: "Hips" });
    for (const s of sizes) {
      const n = normalizeSizeLabel(s);
      ...
      const row = { size: s, [mcol.key]: stdMarketLabel(std.us, family, gender, market), bust: std.bust, waist: std.waist };
      if (gender === "women") row.hip = std.hip;
      rows.push(row);
    }
  ...
  const guide = {
    ...
    note:
      family === "shoes"
        ? "Standard size conversion. Measure your foot length and pick the size that fits."
        : "Standard size chart based on body measurements. Use it as a guide — individual styles may vary.",
    columns,
    rows,
