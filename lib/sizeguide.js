...
const KIND_RULES = [
  { kind: "shoes", re: /\b(boots?|booties|heels?|sneakers?|sandals?|flats?|mules?|loafers?|pumps?|shoes?|footwear|trainers?|slides?|wedges?|espadrilles?|mary jane|oxfords?|brogues?|clogs?|slippers?|moccasins?)\b/ },
...
export function checkGuide(guide, product, market) {
  ...
  if (missing.length) {
    issues.push(`variantmaten zonder rij: ${missing.slice(0, 6).join("/")}`);
    bump(missing.length >= Math.ceil(sizes.length / 2) ? "error" : "warn");
  }
  const extra = guide.rows.filter((r) => !sizes.map(normalizeSizeLabel).includes(normalizeSizeLabel(r.size)));
  if (extra.length && extra.length >= guide.rows.length / 2) {
    issues.push(`tabel heeft ${extra.length} rij(en) die het product niet verkoopt`);
    bump("warn");
  }
  // Geslacht en markt
...
export function productSummary(product) {
  ...
  const images = (product.images || []).map((im) => im.src).filter(Boolean);
  return {
    id: product.id,
    title: product.title || "",
...
export function buildGuide({ chart, product, market, source = "aliexpress", confidence = null }) {
  ...
  if (al.mode === "none" || al.rows.length < Math.ceil(sizes.length * 0.6)) {
    return {
      ok: false,
      reason: `tabelmaten (${chart.rows.map((r) => r.size).slice(0, 6).join("/")}) passen niet op de productmaten (${sizes.slice(0, 6).join("/")})`,
      issues: [],
      missing: al.missing,
    };
  }
  ...
  const guide = {
    v: SG_VERSION,
    status: source === "image" ? "manual" : "aliexpress",
    kind,
    market,
...
const STD_SHOES_MEN = {
  ...
  12: { eu: "46", uk: "11.5", foot: 29.2 }, 13: { eu: "47", uk: "12.5", foot: 30.0 }, 14: { eu: "48", uk: "13.5", foot: 30.8 },
};
...
export function standardGuide(product, market) {
  ...
  if (family === "shoes") {
    ...
    for (const s of sizes) {
      ...
      if (!letter && gender === "women" && has(STD_WOMEN_BY_US, n)) letter = STD_WOMEN_BY_US[n];
      ...
      const row = { size: s, [mcol.key]: stdMarketLabel(std.us, family, gender, market) };
  ...
  if (rows.length < Math.ceil(sizes.length * 0.6)) {
    return { ok: false, reason: `geen standaardtabel voor deze maten (${sizes.slice(0, 6).join("/")})`, missing };
  }
  const guide = {
    v: SG_VERSION,
    status: "standard",
  ...
  const score = missing.length ? Math.max(6, 8 - missing.length) : 8;
  return { ok: true, guide, score, verdict: verdictFor(score), missing, issues: [] };
}
