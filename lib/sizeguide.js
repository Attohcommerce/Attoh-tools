...
export function standardGuide(product, market) {
  ...
  if (family === "shoes") {
    ...
    for (const s of sizes) {
      ...
      // Dames-bereik "8-10" / "12-14" / "16-18": US-nummers → letter(s); twee
      ...
      if (!letter && gender === "women" && /^\d{1,2}\s*[-–\/]\s*\d{1,2}$/.test(n)) {
        ...
        const letters = parts.map((x) => STD_WOMEN_BY_US[x]).filter(Boolean);
        if (letters.length === parts.length) {
          const lo = table[letters[0]];
          const hi = table[letters[letters.length - 1]];
          const row = { size: s, [mcol.key]: stdMarketLabel(parts.join("/"), family, gender, market) };
