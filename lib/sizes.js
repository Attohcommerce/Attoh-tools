...
// BRONSYSTEEM VAN KALE NUMMERS (09-09, LadyGlam Boutique = AUS+NZ met US-
// maten): nummers zónder label (6/10/14, schoen 5–11, "S(4-6)") zijn uit
// het nummer alleen niet te herkennen. Daarom kan de gebruiker per store
// zeggen wat kale nummers zijn (bareSystem: "us" | "uk" | "au" | "eu");
// dan worden ze als dat systeem behandeld en netjes naar de doelmarkt
// omgerekend, incl. letter-labels met nummers ("S(4-6)" → "S(8-10)") en
// bereiken ("8-10" → "12-14"). Zonder bareSystem = het oude gedrag: alleen
// ondubbelzinnig EU en gelabelde mixen worden aangepakt.

export const BARE_SYSTEMS = [
  { key: "", label: "automatisch", info: "alleen EU-nummers en labels als \"EU 38/US 7.5\" worden omgerekend; kale nummers blijven staan" },
  { key: "us", label: "US", info: "kale nummers zijn US-maten (6/10/14, schoenen 5–11) — meest gangbaar bij import van US-stores/AliExpress" },
  { key: "uk", label: "UK", info: "kale nummers zijn UK-maten (8/12/16, damesschoenen 3–8)" },
  { key: "au", label: "AU", info: "kale nummers zijn AU-maten (zelfde nummers als UK voor kleding; damesschoenen als US, herenschoenen als UK)" },
  { key: "eu", label: "EU", info: "kale nummers zijn EU-maten (36/38/40, schoenen 36–45)" },
];
export function marketSystem(market) {
  return market === "UK" ? "uk" : market === "AUS+NZ" ? "au" : market ? "us" : "";
}
 ...
 */
const LETTER_NUM_RE = /^([0-9]?X{0,3}[SML](?:\s*\/\s*[0-9]?X{0,3}[SML])?)\s*[-–:]?\s*(\(?)\s*(?:(US|UK|AU|EU)\s*)?(\d{1,2}(?:\s*[-–\/]\s*\d{1,2})?)\s*\)?$/i;
const RANGE_RE = /^(\d{1,2}(?:\.5)?)\s*[-–]\s*(\d{1,2}(?:\.5)?)$/;

/* Kale nummers: is dit een taille in inches (24–50, overal gelijk) of een
   kledingmaat (dress size)? Dames-bottoms tot en met 20 = kledingmaat. */
function bareIsWaistInches(n, family, gender) {
  if (family !== "bottoms") return false;
  if (gender === "men") return n >= 24 && n <= 50;
  return n >= 22 && n <= 50;
}

/* Eén kaal nummer volgens het opgegeven bronsysteem omrekenen naar het
   doelmarkt-nummer. null = niet omrekenen (zelfde systeem / inches / bh). */
function convertBare(n, family, gender, market, bareSystem) {
  if (!bareSystem || family === "bra") return null;
  if (bareSystem === marketSystem(market) && bareSystem !== "eu") return null;
  if (bareIsWaistInches(n, family, gender)) return null;
  if (gender === "men" && family !== "shoes") return null; // heren = inches, overal gelijk
  const parts = { eu: null, us: null, uk: null, au: null, bare: null, cup: "", labeled: true };
  parts[bareSystem] = n;
  const r = resolveFromParts(parts, family, gender, market, null);
...
export function resolveSizeValue(raw, opts = {}) {
  ...
  const bareSystem = String(opts.bareSystem || "").toLowerCase();
  const src = String(raw == null ? "" : raw).trim();
  const keep = { value: src, changed: false, ok: true };
  if (!market) return keep;
  // Pure lettermaten ("2XL", "XS/S", "3XL") — de 2 of 3 is géén nummer
  if (/^(\d?X{0,4}[SML])(\s*\/\s*\d?X{0,4}[SML])*(\s*\(.*\))?$/i.test(src) && !/\d\s*[-–\/)]|\(\s*(US|UK|AU|EU)?\s*\d/i.test(src)) return keep;

  /* Letter-label met nummer(s): "S(4-6)", "XL (16)", "M - 8/10" → letter
     laten staan, nummers omrekenen (alleen met een bronsysteem). */
  if (family !== "bra" && family !== "shoes") {
    const lm = src.match(LETTER_NUM_RE);
    const sys = lm ? (lm[3] ? lm[3].toLowerCase() : bareSystem) : "";
    if (lm && sys) {
      const nums = lm[4].split(/[-–\/]/).map((x) => Number(x.trim()));
      const conv = nums.map((n) => convertBare(n, family, gender, market, sys));
      if (conv.every((c) => c == null)) {
        // zelfde systeem als de markt: alleen het label opschonen ("S(US 4)" → "S(4)")
        const clean = lm[3] ? `${lm[1].trim()}${lm[2] ? "(" : " "}${lm[4].replace(/\s+/g, "")}${lm[2] ? ")" : ""}` : src;
        return { value: clean, changed: clean !== src, ok: true };
      }
      if (conv.some((c) => c == null)) return { value: src, changed: false, ok: false };
      const [lo, hi] = saneRange(family, gender, market);
      if (conv.some((c) => c < lo || c > hi)) return { value: src, changed: false, ok: false };
      const sep = /–/.test(lm[4]) ? "–" : /\//.test(lm[4]) ? "/" : "-";
      const numTxt = conv.map(fmtNum).join(sep);
      const outVal = lm[2] ? `${lm[1].trim().toUpperCase()}(${numTxt})` : `${lm[1].trim().toUpperCase()} ${numTxt}`;
      return { value: outVal, changed: outVal !== src, ok: true };
    }
    // Bereik zonder letter: "8-10" → "12-14"
    const rm = bareSystem ? src.match(RANGE_RE) : null;
    if (rm) {
      const conv = [Number(rm[1]), Number(rm[2])].map((n) => convertBare(n, family, gender, market, bareSystem));
      if (conv.every((c) => c == null)) return keep;
      if (conv.some((c) => c == null)) return { value: src, changed: false, ok: false };
      const [lo, hi] = saneRange(family, gender, market);
      if (conv.some((c) => c < lo || c > hi)) return { value: src, changed: false, ok: false };
      const outVal = conv.map(fmtNum).join("-");
      return { value: outVal, changed: outVal !== src, ok: true };
    }
  }
  ...
  /* Kaal nummer + bronsysteem → als gelabeld behandelen */
  if (bareSystem && !p.labeled && p.bare != null && family !== "bra") {
    const n = p.bare;
    if (!bareIsWaistInches(n, family, gender) && !(gender === "men" && family !== "shoes")) {
      if (bareSystem === "eu" || bareSystem !== marketSystem(market)) {
        p[bareSystem] = n;
        p.bare = null;
        p.labeled = true;
      }
...
export function analyzeSizes(values, opts = {}) {
  ...
  let flag = anyLabel || (bareEu.length >= minHits && bareEu.length >= numeric.length * 0.7);
  // Bronsysteem opgegeven: alles wat daardoor zou veranderen is een hit
  const bareSystem = String(opts.bareSystem || "").toLowerCase();
  if (!flag && bareSystem && market) {
    const pre = resolveSizeSet(raws, { family, gender, market, bareSystem });
    if (pre.some((r) => r.changed || !r.ok)) flag = true;
  }
  ...
  const setRes = resolveSizeSet(raws, { family, gender, market, bareSystem });
