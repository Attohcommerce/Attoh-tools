...
const ALPHA = 0.55; // demping: toppers winnen, maar slokken niet alles op
const CAP = 28; // max producten per keyword
const FLOOR = 5; // minder dan dit → keyword valt af
const W_SHARE = 0.87; // dames-aandeel bij man + vrouw
const COL_CAP_FRAC = 0.3; // geen collectie groter dan 30% van het totaal
...
function waterfill(pool, budget) {
  // Verdeel budget ∝ season^ALPHA met harde cap; wat boven de cap uitkomt
  // schuift door naar de rest.
  const w = pool.map((r) => Math.pow(r.season, ALPHA));
...
function allocateKeywords(items, budget) {
  // Max spreiding: zoveel mogelijk keywords meenemen zolang de laagste
  // nog >= FLOOR producten krijgt (binary search op het aantal).
  const sorted = [...items].sort((a, b) => b.season - a.season);
  if (!sorted.length || budget < FLOOR) return [];
  let lo = 1;
  let hi = Math.min(sorted.length, Math.floor(budget / FLOOR));
  let best = null;
  while (lo <= hi) {
    const K = (lo + hi) >> 1;
    const pool = sorted.slice(0, K);
    const alloc = waterfill(pool, budget);
    if (Math.min(...alloc) >= FLOOR - 0.5) {
      best = { pool, alloc };
      lo = K + 1;
    } else {
      hi = K - 1;
  ...
  if (!best) {
    ...
    best = { pool, alloc: waterfill(pool, budget) };
  }
  const out = best.pool.map((r, i) => ({
    ...r,
    n: Math.min(CAP, Math.max(FLOOR, Math.round(best.alloc[i]))),
  }));
...
function allocateByCollection(items, budget) {
  // Laag 1: budget over collecties ∝ vraag^ALPHA met 30%-cap.
  const byCol = new Map();
  ...
  const w = new Map(
    cols.map((c) => [c, Math.pow(byCol.get(c).reduce((s, x) => s + x.season, 0), ALPHA)])
  );
  const alloc = new Map(cols.map((c) => [c, 0]));
  const capped = new Set();
  const colCap = budget * COL_CAP_FRAC;
  for (let iter = 0; iter < 60; iter++) {
    const free = cols.filter((c) => !capped.has(c));
    const used = [...capped].reduce((s, c) => s + alloc.get(c), 0);
    const rem = budget - used;
    const sw = free.reduce((s, c) => s + w.get(c), 0);
    if (!free.length || sw <= 0) break;
    let changed = false;
    for (const c of free) alloc.set(c, (rem * w.get(c)) / sw);
    for (const c of free) {
  ...
  let leftover = 0;
  for (const c of cols) {
    const b = alloc.get(c) + leftover;
    if (b < FLOOR) {
      leftover = b; // te weinig voor deze collectie — schuift door
      continue;
    }
    const got = allocateKeywords(byCol.get(c), Math.round(b));
    if (!got.length) {
      leftover = b;
      continue;
    }
...
export function buildVerdeling(rows, opts = {}) {
  ...
  const total = Number(opts.total) || TOTAL_DEFAULT;
  ...
  if (genders === "MV") {
    ...
    allRows = [
      ...allocateByCollection(unique.filter((r) => r.g === "V"), wBudget),
      ...allocateByCollection(unique.filter((r) => r.g === "M"), total - wBudget),
    ];
  } else {
    allRows = allocateByCollection(unique, total);
  }
  allRows = allRows.filter((x) => x.n >= FLOOR);
