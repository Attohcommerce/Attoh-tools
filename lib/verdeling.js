...
const COL_SEASON_CURVE = {
  ...
  // shirts, schoenen, gelegenheidsjurken) en krijgt geen correctie.
};
...
const ITEM_SEASON = [
  ...
  { name: "hard zomer",
    re: /\b(swim|swimming|board ?shorts?|boardies|bathing|beach|linen|seersucker|sandals?|thongs?|flip ?flops?|slides?|espadrilles?|tank|singlet|sleeveless|short sleeve|resort|poolside|sun ?dress|rash ?(guard|vest))\b/,
...
export function seasonCapFrac(score) {
  ...
  return Math.max(0.012, score * 0.22);
}
...
export function seasonFloorFrac(col, score) {
  if (!COL_SEASON_CURVE[col]) return null;
  if (score < 0.72) return null;
  return Math.min(0.09, (score - 0.6) * 0.25);
}
...
export function collectionFor(kw) {
  ...
  if (has(kw, "swim shorts?", "swimming shorts?", "board ?shorts?", "boardies", "swim trunks?", "bathing suits?", "swim briefs?", "rash ?(guards?|vests?)"))
...
const COL_TYPES = {
  ...
  Swimwear: ["swimwear", "shorts"],
  "Men's Swimwear": ["swimwear", "shorts"],
};
...
const TYPE_HOME = {
  ...
  loafers: "Flats & Loafers", swimwear: "Swimwear",
};
...
const MEN_NO_HOME = new Set(["Casual Dresses", "Skirts"]);
...
export function buildVerdeling(rows, opts = {}) {
  ...
  for (const r of rows) {
    ...
    const sScore = market ? seasonScore(col, windowSeasons, kw) : 1;
    ...
    if (market && sScore < SEASON_DEAD) {
      stats.offSeason = (stats.offSeason || 0) + 1;
      continue;
    }
    const sFactor = market ? 0.25 + 1.05 * sScore : 1;
  ...
  const rules = market
    ? {
        ...
        cap: (col) => {
          ...
          const sc = seasonCapFrac(seasonScore(col, windowSeasons, null));
        ...
        floor: (col) => {
          ...
          const sf = seasonFloorFrac(col, seasonScore(col, windowSeasons, null));
