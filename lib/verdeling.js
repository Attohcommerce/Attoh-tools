...
export function fixKeywordOrder(kw) {
  ...
  if (!glued && t.length >= 2 && t.length <= 4 && !SHOE_TAIL_RE.test(t[t.length - 1]) && !isTypePhrase(s)) {
...
const MALE_LEANING_PAIRS = [
  ["cargo", "short"], ["chino", "short"], ["work", "short"], ["board", "short"], ["swim", "short"],
...
export function genderOf(kw, storeGenders) {
  ...
  if (men) return "M";
  if (!women && (MALE_LEANING_RE.test(kw) || maleLeaningPair(kw)) && !FEMALE_ITEM_RE.test(kw)) return "M";
  return "V";
}
