...
export function buildVerdeling(rows, opts = {}) {
  ...
  for (const x of allRows) {
    ...
    /* Planner-omkeringen met het geslacht achteraan: "suede coat mens",
       ...
       "mens suede coat". */
    /* Alle geslachtswoorden, niet alleen "mens"/"womens": "linen shirt male",
       "shorts for guys", "top ladies" belandden anders letterlijk zo in de
       producttitel (Shapes-run 18-9). Canon voegde ze al samen; dit zet ook
       de tekst goed. */
    const last = t[t.length - 1];
    if (t.length >= 2 && GENDER_WORD_RE.test(last) && !GENDER_WORD_RE.test(t[0])) {
      const g = MALE_WORD_RE.test(last) ? "mens" : "womens";
      /* "loafers for men" → "mens loafers", niet "mens loafers for": het
         voorzetsel vóór het geslachtswoord gaat mee weg. */
      let head = t.slice(0, -1);
      if (head.length >= 2 && /^(for|voor)$/i.test(head[head.length - 1])) head = head.slice(0, -1);
      x.kw = [g, ...head].join(" ");
      t = x.kw.split(/\s+/);
    }
    /* Drie woorden met het geslacht voorop: "mens pants cargo" → "mens cargo
       pants". Alleen als het paar erachter omgedraaid wél een producttype is. */
    if (t.length === 3 && /^(mens|womens)$/i.test(t[0]) && !SHOE_TAIL_RE.test(t[2])) {
      try {
        const straight = analyzeKeyword(`${t[1]} ${t[2]}`);
        const flipped = analyzeKeyword(`${t[2]} ${t[1]}`);
        if (!(straight && straight.typeId) && flipped && flipped.typeId) {
          x.kw = `${t[0]} ${t[2]} ${t[1]}`;
          t = x.kw.split(/\s+/);
        }
      } catch {}
    ...
    /* Schoenwoord achteraan is altijd het hoofdwoord. fashion.js kent "shoes"
       niet als type, dus werd "dress shoes" (nu een herenkeyword) omgedraaid
       tot "shoes dress" — in een producttitel voor herenschoenen. */
    if (SHOE_TAIL_RE.test(t[1])) continue;
    try {
