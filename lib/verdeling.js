...
const MODES = {
  spread: {
    ...
    materialCap: 0.14, // max aandeel keywords per stof (linnen, denim, leer …)
    colorShare: 0.2, // max aandeel keywords dat een kleurvariant is
  },
  focus: {
    alpha: 1.0,
    cap: 28,
...
export function isVerdelingJunk(kw) {
  ...
  if (AU_RETAIL_RE.test(kw)) return "merkenlijst";
  // "size 15 mens shoes", "size 22 dresses" — daar richt je geen collectie op in
  if (/\bsizes?\s+\d/.test(kw) || /\b(plus|petite)\s+size\s+\d/.test(kw)) return "maatspecifiek";
  if (!DRESS_ITEM_RE.test(kw)) {
    if (PANTS_DRESS_LOOSE_RE.test(kw)) return "tegenstrijdig";
    if (BOTTOMS_CLASH_RE.test(kw)) return "tegenstrijdig";
  }
  /* Vanaf VIER woorden: "short sleeve button up with shorts" is twee keywords
     aan elkaar, maar "dresses with sleeves" is een echte zoekvraag. */
  if (kw.split(/\s+/).length >= 4 && GLUE_PHRASE_RE.test(kw)) return "plakzin";
  return null;
}
...
/* Dezelfde botsing met woorden ertussen: "linen pants women's dress" (6
   producten in de Shapes-run 25-9) is een broek én een jurk. "dress pants"
   en "dress shirt" blijven staan — die worden hieronder uitgezonderd. */
const PANTS_DRESS_LOOSE_RE = /\b(pants|trousers|slacks|jeans|shorts|leggings|skirts?)\b.*\bdress(es)?$/;

/* Twee verschillende onderstukken in één keyword: "pants shorts" is geen
   artikel maar een Planner-plakwerk. */
const BOTTOMS_CLASH_RE =
  /\b(pants|trousers|slacks)\b[^.]*\b(shorts|skirts?)\b|\b(shorts|skirts?)\b[^.]*\b(pants|trousers|slacks)\b/;

/* Australische/NZ retailketens die de merkenlijst mist omdat het gewone
   woorden zijn: "cotton on shorts", "cotton on linen pants" en "cotton on
   tank tops" stonden samen voor 23 producten in de run van 25-9. */
const AU_RETAIL_RE =
  /\bcotton on\b|\bbig ?w\b|\bbest ?& ?less\b|\bbest and less\b|\bdotti\b|\bsportsgirl\b|\bcity ?beach\b|\bthe iconic\b|\bsurfstitch\b|\bmyer\b|\bdavid jones\b|\brebel sport\b|\bnoni ?b\b|\bkaties\b|\brivers\b|\btarocash\b|\byd\.? ?(australia)?\b|\bconnor\b/;

/* Lange Planner-plakzinnen met een verbindingswoord: "short sleeve button up
   with shorts" (5 producten) is geen zoekvraag maar twee keywords aan elkaar. */
const GLUE_PHRASE_RE = /\b(with|and|for the|to wear)\b/;
...
/* Woorden die ACHTERAAN altijd het hoofdwoord zijn, ook al kent fashion.js ze
   niet als producttype. Zonder deze lijst draaide de volgorde-correctie
   "dress shoes" om tot "shoes dress" en "bikini cover up" tot "cover up
   bikini". */
const TAIL_NOUNS_RE =
  /^(shoes?|boots?|sneakers?|trainers?|sandals?|heels?|loafers?|slides?|thongs?|bags?|totes?|purses?|clutch(es)?|backpacks?|hats?|caps?|beanies?|visors?|sets?|suits?|swimwear|up|wear|jeans|pants|shorts|skirts?)$/i;
...
/* Schrijffouten die letterlijk in producttitels belanden. "mens lino pants"
   stond in de Shapes-run 25-9 met 28 producten bovenaan Men's Trousers —
   lino is vloerbedekking, linen is de stof. */
const SPELLING = { lino: "linen", lenin: "linen", linnen: "linen", trouser: "trousers", tshirts: "t shirts" };

/* Plaats- en landnamen uit Planner-frasen. Ze zeggen niets over het product
   en stonden letterlijk in titels ("cocktail dresses aus", "formal brisbane
   gowns"). */
const GEO_WORDS = new Set(
  `aus au australia australian nz nzl newzealand uk gb usa us america american
   canada canadian ireland irish sydney melbourne brisbane perth adelaide
   canberra hobart darwin auckland wellington christchurch london dublin`
    .split(/\s+/)
    .filter(Boolean)
);

/* Eén stam, twee woorden: "mens trouser pants" (17 producten) is geen
   artikel maar een Planner-stapel. Het laatste woord is het hoofdwoord, dus
   de eerdere dubbel verdwijnt: "mens trouser pants" → "mens pants". */
function dropDuplicateStems(tokens) {
  const stemOf = (t) => {
    let x = synonymOf(t) || t;
    x = x.replace(/(sses|shes|ches|xes)$/, (m) => m.slice(0, -2));
    if (x.length > 3 && x.endsWith("s") && !x.endsWith("ss")) x = x.slice(0, -1);
    return x;
  };
  /* "short" is ook een bijvoeglijk naamwoord: in "short sleeve button up
     with shorts" mag de eerste niet verdwijnen. Zelfde voor "dress"/"top". */
  const AMBIGUOUS = new Set(["short", "dress", "top", "tank", "crop", "long"]);
  const seen = new Map();
  tokens.forEach((t, i) => {
    const st = stemOf(t);
    if (!AMBIGUOUS.has(st)) seen.set(st, i); // laatste positie wint
  });
  return tokens.filter((t, i) => {
    const st = stemOf(t);
    return !seen.has(st) || seen.get(st) === i;
  });
}
...
export function fixKeywordOrder(kw) {
  ...
  if (!s) return s;
  /* Apostrof weg: "men's" en "women's" werden niet als geslachtswoord herkend,
     waardoor "pajama pants men's" als "pants men's pajama" in de titel belandde. */
  s = s.replace(/[\u2018\u2019'`]/g, "");
  /* Land- en stadsnamen zijn navigatie, geen producteigenschap: "cocktail
     dresses aus" en "formal brisbane gowns" horen als "cocktail dresses" en
     "formal gowns" in een producttitel. */
  s = s
    .split(" ")
    .filter((t) => !GEO_WORDS.has(t))
    .join(" ")
    .trim() || s;
  // Kale "size" achteraan is een Planner-restant: "mens pants size" → "mens pants"
  s = s.replace(/\s+sizes?$/, "").trim() || s;
  s = s.split(" ").map((t) => (OWN.call(SPELLING, t) ? SPELLING[t] : t)).join(" ");
  {
    const dedup = dropDuplicateStems(s.split(" "));
    if (dedup.length >= 2) s = dedup.join(" ");
  }
  ...
  /* Geslachtswoord hoort voorop, waar het ook staat: "suede coat mens" →
     "mens suede coat", "loafers for men" → "mens loafers", en ook midden in
     de zin: "pants for men white" → "mens pants white" (daarna zet de
     volgende stap er "mens white pants" van). Het voorzetsel ervóór gaat
     mee weg. */
  const gi = t.findIndex((w) => GENDER_WORD_RE.test(w));
  if (t.length >= 2 && gi > 0) {
    const g = MALE_WORD_RE.test(t[gi]) ? "mens" : "womens";
    const drop = new Set([gi]);
    if (/^(for|voor)$/i.test(t[gi - 1] || "")) drop.add(gi - 1);
    const rest = t.filter((_, i) => !drop.has(i));
    if (rest.length) {
      s = [g, ...rest].join(" ");
      t = s.split(" ");
    }
  ...
  /* Hoofdwoord naar achteren. fashion.js herkent een producttype alleen als
     ...
     "shoes dress". */
  /* Zinnen met een verbindingswoord niet omgooien: "dresses with sleeves"
     werd anders "with sleeves dresses". */
  const glued = t.some((w) => /^(with|and|or|for|to|in|on|over|under|without)$/.test(w));
  if (!glued && t.length >= 2 && t.length <= 4 && !TAIL_NOUNS_RE.test(t[t.length - 1]) && !isTypePhrase(s)) {
  ...
  // "palazzo" en "palazzo clothing" zijn broeken; zonder dit blijft er een
  // kaal bijvoeglijk naamwoord als producttitel over.
  if (/(^|\s)palazzo$/.test(s)) s = `${s} pants`;
  return s;
}
...
/* Dezelfde herenartikelen, maar als WOORDPAAR in plaats van als frase.
   MALE_LEANING_RE kent alleen "cargo shorts": "baggy cargo shorts" werd M,
   "cargo baggy shorts" werd V — zelfde artikel, twee rijen, twee collecties
   (Shapes-run 25-9). Ook "polo jersey shirt" viel zo naar Blouses. */
const MALE_LEANING_PAIRS = [
  ["cargo", "short"], ["cargo", "pant"], ["chino", "short"], ["bathing", "short"], ["swim", "trunk"], ["work", "short"], ["board", "short"], ["swim", "short"],
  ["polo", "shirt"], ["polo", "top"], ["polo", "tshirt"], ["polo", "tee"],
  ["button", "shirt"], ["collared", "shirt"], ["flannel", "shirt"], ["hawaiian", "shirt"],
  ["rugby", "shirt"], ["fishing", "shirt"], ["work", "shirt"], ["work", "pant"],
  ["dress", "shoe"], ["boat", "shoe"], ["derby", "shoe"], ["oxford", "shoe"],
  ["bomber", "jacket"], ["overshirt", ""], ["henley", ""], ["singlet", ""],
];

function maleLeaningPair(kw) {
  const set = new Set();
  for (let t of String(kw || "").toLowerCase().split(/[^a-z0-9]+/)) {
    if (!t) continue;
    t = t.replace(/(sses|shes|ches|xes)$/, (m) => m.slice(0, -2));
    if (t.length > 3 && t.endsWith("s") && !t.endsWith("ss")) t = t.slice(0, -1);
    set.add(t);
  }
  for (const [a, b] of MALE_LEANING_PAIRS) if (set.has(a) && (!b || set.has(b))) return true;
  return false;
}

/* HERKOMST van het geslacht, niet alleen de uitkomst. Vier bronnen:
     woord   — er staat letterlijk "mens"/"womens"/"ladies"/… in het keyword
     store   — single-gender store: de store ís het geslacht
     type    — het artikel zelf is eenduidig (jurk/rok/bikini = V, polo/chino = M)
     default — géén signaal; dan valt het terug op de store-instelling
   Die laatste categorie was de bron van de meeste fouten: "short sleeve
   shirts" en "linen white shorts" vielen stil naar dames omdat er geen
   heren-woord in stond, terwijl "mens short sleeve shirts" wél M werd. Nu
   staat de herkomst per rij in de sheet, zodat een twijfelgeval zichtbaar is
   in plaats van stilzwijgend verkeerd. `neutralGender` in het store-profiel
   zet de default om voor een store die heren-zwaar is. */
export function genderInfo(kw, storeGenders, opts = {}) {
  const g = storeGenders === "M" || storeGenders === "V" ? storeGenders : "MV";
  const women = WOMEN_RE.test(kw);
  const men = MEN_RE.test(kw) && !women;
  if (g === "M") return women ? { g: "V", src: "woord" } : { g: "M", src: men ? "woord" : "store" };
  if (g === "V") return men ? { g: "M", src: "woord" } : { g: "V", src: women ? "woord" : "store" };
  if (men) return { g: "M", src: "woord" };
  if (women) return { g: "V", src: "woord" };
  if (FEMALE_ITEM_RE.test(kw)) return { g: "V", src: "type" };
  if (MALE_LEANING_RE.test(kw) || maleLeaningPair(kw)) return { g: "M", src: "type" };
  const neutral = opts.neutralGender === "M" ? "M" : "V";
  return { g: neutral, src: "default" };
}

export function genderOf(kw, storeGenders, opts) {
  return genderInfo(kw, storeGenders, opts).g;
}
...
export function collectionFor(kw, opts = {}) {
  ...
  const gi = genderInfo(kw, opts.storeGenders, opts);
  const g = gi.g;
  const gSrc = gi.src;
  const men = g === "M";
  ...
  if (has(kw, "swim shorts?", "swimming shorts?", "board ?shorts?", "boardies", "swim trunks?", "bathing suits?", "swim briefs?", "rash ?(guards?|vests?)", "rashies"))
    return { col: men ? "Men's Swimwear" : "Swimwear", g, gSrc };
  ...
  if (has(kw, "bikinis?", "tankinis?", "swimsuits?", "swimwear", "swimmers", "bathers", "cossies?", "togs",
    ...
    return { col: men ? (has(kw, "kaftans?", "caftans?", "sarongs?", "cover ?ups?") ? null : "Men's Swimwear") : "Swimwear", g, gSrc };
  ...
  if (has(kw, "sandals?", "sliders?", "slides?", "flip ?flops?", "thongs?", "espadrilles?"))
    return { col: men ? "Men's Sandals" : "Sandals", g, gSrc };
  ...
  if (has(kw, "loafers?", "flats", "ballet (flats|pumps)", "mary janes?", "moccasins?"))
    return { col: men ? "Men's Shoes" : "Flats & Loafers", g, gSrc };
  ...
  if (
    ...
    return { col: men ? "Men's Shoes" : "Shoes", g, gSrc };
  ...
  if (has(kw, "jumpsuits?", "playsuits?", "rompers?", "overalls?", "dungarees"))
    return { col: men ? null : "Jumpsuits & Playsuits", g, gSrc };
  if (
    ...
    return { col: men ? null : "Co-ord Sets", g, gSrc };
  // Singlet = Australisch voor tanktop/hemd
  if (has(kw, "singlets?")) return { col: men ? "Men's Shirts" : "Tank Tops", g, gSrc };
  ...
  if (!dressItem) {
    if (
      ...
      return { col: men ? null : "Wedding Guest & Bridesmaid Dresses", g, gSrc };
    if (has(kw, "graduation")) return { col: men ? null : "Graduation Dresses", g, gSrc };
    if (has(kw, "maternity")) {
      if (has(kw, "dress(es)?")) return { col: men ? null : "Maternity Dresses", g, gSrc };
      return { col: null, g, gSrc };
    }
    ...
    if (has(kw, "race day", "races", "racewear", "melbourne cup", "derby", "racing carnival"))
      return { col: men ? null : "Formal & Occasion Dresses", g, gSrc };
    if (
    ...
    if (has(kw, "dress(es)?", "sundress(es)?"))
      return { col: men ? null : "Casual Dresses", g, gSrc };
  }
  ...
  if (
    ...
    return { col: men ? "Men's Jackets & Coats" : "Jackets & Coats", g, gSrc };
  ...
  if (men && has(kw, "vests?") && !has(kw, "sweater vests?", "knit vests?", "cardigan vests?", "vest tops?"))
    return { col: "Men's Jackets & Coats", g, gSrc };

  // Broeken
  if (has(kw, "jeans", "denim")) {
    if (!has(kw, "jackets?", "shirts?", "skirts?", "dress(es)?"))
      return { col: men ? "Men's Jeans" : "Jeans", g, gSrc };
  }
  if (
    has(kw, "trousers?", "pants", "chinos?", "joggers?", "leggings?", "culottes?",
      "palazzo", "wide leg", "khakis?", "slacks", "sweatpants?", "sweat pants", "corduroys")
  )
    return { col: men ? "Men's Trousers" : "Trousers", g, gSrc };
  if (has(kw, "shorts")) return { col: men ? "Men's Shorts" : "Shorts", g, gSrc };
  if (has(kw, "skirts?", "skorts?")) return { col: men ? null : "Skirts", g, gSrc };

  // Tops
  if (has(kw, "hoodies?", "sweatshirts?", "zip ups?"))
    return { col: men ? "Men's Hoodies" : "Hoodies", g, gSrc };
  // quarter/half zip en fleece (pullover) zijn knitwear-midlayers — de 55+
  // herenkern (375k/seizoen voor "mens quarter zip" alleen al).
  if (
    has(kw, "jumpers?", "sweaters?", "cardigans?", "knitwear",
      "turtlenecks?", "roll necks?", "pullovers?", "quarter zips?", "half zips?", "zip necks?", "fleeces?") ||
    /* "knit" alleen is een STOF: "knit top" is een top, geen trui. Zonder deze
       uitzondering belandde "tops short sleeve knit" in Sweaters — een
       uit-seizoen collectie in een Australische zomerrun. */
    (has(kw, "knits?", "knitted") && !has(kw, "tops?", "tanks?", "vests?", "dress(es)?", "skirts?", "shirts?", "polos?"))
  )
    return { col: men ? "Men's Sweaters" : "Sweaters", g, gSrc };
...
const COMPOUNDS = [
  /* "button up shirts", "button down shirt" en "button shirt" zijn één
     artikel; in de Shapes-run van 25-9 stonden ze als twee rijen met 19
     producten samen. "up" en "down" mogen NIET in FILLER — "down jacket"
     is een donsjas. */
...
const SYNONYMS = {
  ...
  trouser: "pant", trousers: "pant", pants: "pant", slack: "pant", slacks: "pant",
...
/* Kleuren en stoffen zijn EIGENSCHAPPEN, geen productbehoeftes. Ze staan
   hier op moduleniveau omdat ze op twee plekken nodig zijn: het plafond per
   productsoort (hieronder) en het plafond over de hele store (4d). */
const COLOR_WORDS = new Set(
  `black white ivory cream gold golden silver red burgundy wine blue navy cobalt
   ...
   yellow mustard orange rust terracotta camo`
    .split(/\s+/)
    .filter(Boolean)
);

/* Stoffen. In de Shapes-run van 25-9 was 22% van de store linnen: linen
   pants, white linen pants, mens linen pants, linen shirt, white linen
   shirt, mens linen shirt, linen tops, linen dress, white linen gown,
   linen white shorts, linen mens shorts. Elk keyword op zich klopt; samen
   is het één stof over een vijfde van de winkel. */
const MATERIAL_WORDS = new Set(
  `linen denim leather suede cotton satin silk knit knitted wool woolen cashmere
   corduroy cord velvet mesh lace chiffon fleece sequin sequined ribbed crochet
   tulle nylon polyester jersey`
    .split(/\s+/)
    .filter(Boolean)
);
...
/* Accessoires zijn geen maatgevoelig kernassortiment. Een tassen- of
   hoedencollectie heeft geen twintig varianten van hetzelfde nodig. */
const ACCESSORY_COLS = new Set(["Bags", "Men's Bags", "Hats", "Men's Hats"]);

function kwCapOf(r, P) {
  let cap = r && r.head ? P.headCap : P.cap;
  /* DIEPTE. Eén zoekterm mag geen hele collectie dragen: "mens cross body
     bag" was in de Shapes-run van 25-9 de enige heren-tasterm en pakte de
     volle cap van 28 producten. Die 28 verschillende crossbody's bestaan
     niet — de vraag erachter is één term breed. */
  const kws = r && r.colKws ? r.colKws : 3;
  if (kws <= 1) cap = Math.round(cap * 0.5);
  else if (kws === 2) cap = Math.round(cap * 0.75);
  if (r && ACCESSORY_COLS.has(r.col)) cap = Math.min(cap, Math.round(P.cap * 0.45));
  return Math.max(P.floor, cap);
}
...
function allocateByCollection(items, budget, P0, droppedCols, rules) {
  ...
  let P = P0;
  /* Hoeveel zoektermen draagt elke collectie? Die diepte bepaalt mede de cap
     per keyword (zie kwCapOf) en moet dus vóór de capaciteitsberekening
     bekend zijn. */
  {
    const n = new Map();
    for (const r of items) n.set(r.col, (n.get(r.col) || 0) + 1);
    for (const r of items) r.colKws = n.get(r.col) || 1;
  }
...
export function buildVerdeling(rows, opts = {}) {
  ...
  /* Waar valt een kledingstuk ZONDER geslachtswoord naartoe? Standaard dames
     (in mode is dat het grootste deel van het zoekvolume). Een heren-zware
     unisex store zet `neutralGender: "M"` in zijn profiel. */
  const neutralGender = opts.neutralGender || (profile && profile.neutralGender) || "V";
  ...
  const kwFacts = (kw, raw) => {
    const k = mkey(kw);
    let f = memo.get(k);
    if (f) return f;
    if (isVerdelingJunk(kw) || (raw && raw !== kw && isVerdelingJunk(raw))) {
      f = { junk: true };
    } else {
      let { col, g, gSrc } = collectionFor(kw, { storeGenders: genders, neutralGender });
      col = consistentCollection(kw, col);
      f = { junk: false, col, g, gSrc, canon: canonKey(kw, { storeGenders: genders }) };
    }
    memo.set(k, f);
    return f;
  };
  ...
  for (const r of rows) {
    ...
    const rawKw = stripUk(String(r.kw || "").toLowerCase().trim());
    const kw = fixOrder(rawKw);
    if (!kw || seenKw.has(kw)) continue;
    seenKw.add(kw);
    const facts = kwFacts(kw, rawKw);
  ...
  /* 4d: AANDEEL-PLAFOND over de hele store. Het kleurplafond hierboven kijkt
     per productsoort en ving daardoor niets toen dezelfde kleur over tien
     verschillende artikelen verspreid stond: de run van 25-9 had 22% linnen
     en 24% kleurvarianten zonder dat er één plafond afging. Een eigenschap
     mag nooit een vijfde van het assortiment bepalen — je verkoopt dan tien
     keer hetzelfde met een ander etiket. De zwakste keywords per eigenschap
     vallen af; hun budget gaat vanzelf naar de rest. */
  {
    const limit = (n, frac) => Math.max(2, Math.floor(n * frac));
    const cullBy = (bucketOf, frac, label) => {
      ...
      for (const [b, arr] of buckets) {
        const max = limit(unique.length, frac);
        if (arr.length <= max) continue;
        arr.sort((a, x) => x.score - a.score);
        for (const r of arr.slice(max)) drop.add(r);
        stats.attrCapped = stats.attrCapped || [];
        stats.attrCapped.push(`${label} ${b}: ${arr.length} → ${max} keywords`);
      }
      if (drop.size) unique = unique.filter((r) => !drop.has(r));
    };
    // per stof: nooit meer dan P.materialCap van de keywords
    cullBy((r) => r.canon.split(" ").find((w) => MATERIAL_WORDS.has(w)) || null, P.materialCap, "stof");
    // alle kleurvarianten samen: nooit meer dan P.colorShare van de keywords
    cullBy((r) => (r.canon.split(" ").some((w) => COLOR_WORDS.has(w)) ? "kleurvarianten" : null), P.colorShare, "kleur");
  }
  ...
  /* Kern-seizoenscollecties ZONDER ook maar één keyword. Die halen stats.
     colSeason niet, want dat wordt uit de overgebleven keywords opgebouwd —
     precies daarom bleef in de Shapes-run van 25-9 het ontbreken van
     sandalen en schoeisel onzichtbaar, terwijl het in een Australische
     zomerstore de grootste categorie is. */
  {
    const have = new Set(unique.map((u) => u.col));
    const wantMen = genders !== "V";
    const wantWomen = genders !== "M";
    stats.missingCore = [...CORE_SEASON, "Shoes", "Men's Shoes", "Flats & Loafers"]
      .filter((c) => {
        if (have.has(c) || blocked.has(c)) return false;
        const isMen = c.startsWith("Men's");
        if (isMen ? !wantMen : !wantWomen) return false;
        return market ? seasonScore(c, windowSeasons, null) >= 0.72 : false;
      });
  }
